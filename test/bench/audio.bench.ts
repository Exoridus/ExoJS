...
const makeGain = (): GainNode =>
  ({
    ...
    gain: { value: 1, setTargetAtTime: () => undefined, cancelScheduledValues: () => undefined, setValueAtTime: () => undefined, linearRampToValueAtTime: () => undefined },
  }) as unknown as GainNode;

const makeBufferSource = (): AudioBufferSourceNode =>
  ({ connect: () => undefined, disconnect: () => undefined, start: () => undefined, stop: () => undefined, playbackRate: { value: 1 }, loop: false, loopStart: 0, loopEnd: 0, buffer: null, onended: null }) as unknown as AudioBufferSourceNode;
...
const makePanner = (): PannerNode =>
  ({
    connect: () => undefined, disconnect: () => undefined, context: { currentTime: 0 } as AudioContext,
    panningModel: 'equalpower' as PanningModelType, distanceModel: 'linear' as DistanceModelType,
...
const installMocks = (): void => {
  Object.defineProperty(globalThis, 'AudioContext', { configurable: true, writable: true, value: class { constructor() { return makeMockContext(); } } });
  if (typeof (globalThis as Record<string, unknown>)['OfflineAudioContext'] === 'undefined') {
    Object.defineProperty(globalThis, 'OfflineAudioContext', { configurable: true, writable: true, value: class { sampleRate = 44100; decodeAudioData() { return Promise.resolve({} as AudioBuffer); } } });
  }
  if (typeof (globalThis as Record<string, unknown>)['AudioWorkletNode'] === 'undefined') {
    Object.defineProperty(globalThis, 'AudioWorkletNode', { configurable: true, writable: true, value: class { connect = () => undefined; disconnect = () => undefined; parameters = new Map(); port = { postMessage: () => undefined, onmessage: null }; } });
  }
};

const makeAudioBuffer = (duration = 2, sampleRate = 44100): AudioBuffer =>
  ({ duration, sampleRate, numberOfChannels: 1, length: duration * sampleRate, getChannelData: () => new Float32Array(duration * sampleRate) }) as unknown as AudioBuffer;
...
describe('audio', () => {
  ...
  bench('AudioBus filter chain add/remove (5 filters, 100 iterations)', async () => {
    ...
    for (let iter = 0; iter < 100; iter++) {
      ...
      for (const f of filters) { bus.removeFilter(f); f.destroy(); }
    }
