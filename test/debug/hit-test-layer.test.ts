...
const makeApp = (
    root: FakeNode | null = null,
    interaction: any = makeInteraction(),
) => ({
...
describe('HitTestLayer', () => {
    ...
    test('hovered node gets yellow color (1, 1, 0)', () => {
        ...
        jest.spyOn(Graphics.prototype, 'lineColor', 'set').mockImplementation(function (this: unknown, c) {
            colorAssignments.push({ r: (c as import('@/core/Color').Color).r, g: (c as import('@/core/Color').Color).g, b: (c as import('@/core/Color').Color).b });
    ...
    test('captured node gets cyan color (0, 1, 1)', () => {
        ...
        const app = makeApp(
            capturedNode as unknown as FakeNode,
            interaction ,
        );
        const layer = new HitTestLayer(app);
        const backend = app.backend;
