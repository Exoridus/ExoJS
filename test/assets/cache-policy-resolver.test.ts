...
/** Drives one acquisition through `cache`, reporting whether the network was reached. */
function acquire(cache: AssetCache, source = 'url:/a.txt', fetch: () => Promise<string> = () => Promise.resolve('fresh')): Promise<string> {
...
describe('a route resolves its policy per acquisition', () => {
  ...
  test('a resolver is asked once per acquisition, with the namespace and source key', async () => {
    ...
    expect(seen).toEqual([
      { namespace: 'text', sourceKey: 'url:/a.txt' },
      { namespace: 'text', sourceKey: 'url:/b.txt' },
    ]);
  });
  ...
  test('the answer is fixed for the whole acquisition, whatever changes underneath it', async () => {
    ...
    const store = createCacheStoreDouble();
    const cache = new AssetCache({ stores: store, policy: new ConnectivityPolicyResolver(connectivity) });
    ...
    const pending = acquire(cache, 'url:/slow.txt', async () => {
      release!();
    ...
    await expect(acquire(cache, 'url:/other.txt')).rejects.toBeInstanceOf(AssetCacheMissError);
  });
});
...
describe('ConnectivityPolicyResolver', () => {
  ...
  test('an online host acquires and fills the cache', async () => {
    const { cache, store } = setup('online');
  ...
  test('an unknown host still acquires - nothing said the network is gone', async () => {
    const { cache } = setup('unknown');

    await expect(acquire(cache)).resolves.toBe('fresh');
  });

  test('an offline host serves a cached record without touching the network', async () => {
    const { cache, host } = setup('online');
    const fetch = vi.fn(() => Promise.resolve('fresh'));

    await acquire(cache, 'url:/a.txt', fetch);
    host.emit('offline');
    fetch.mockClear();

    await expect(acquire(cache, 'url:/a.txt', fetch)).resolves.toBe('fresh');
  ...
  test('an offline host misses loudly for a record nobody cached, and never fetches', async () => {
    ...
    await expect(acquire(cache, 'url:/missing.txt', fetch)).rejects.toBeInstanceOf(AssetCacheMissError);
    expect(fetch).not.toHaveBeenCalled();
  });
  ...
  test('the two policies it chooses between are configurable', async () => {
    ...
    const cache = new AssetCache({ policy: new ConnectivityPolicyResolver(connectivity, { online: new CacheFirstPolicy(), offline }) });

    await expect(acquire(cache)).resolves.toBe('from the shipped cache');
  });

  test('its default offline policy is cache-only', () => {
    const connectivity = new Connectivity(hintSource('offline').source);
    const resolver = new ConnectivityPolicyResolver(connectivity);

    expect(resolver.policyFor()).toBeInstanceOf(CacheOnlyPolicy);
  });
});
