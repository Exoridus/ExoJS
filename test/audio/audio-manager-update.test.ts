import { getAudioManager, _resetAudioManagerForTesting } from '@/audio/AudioManager';
...
describe('AudioManager.update()', () => {
    ...
    test('Application.update() calls getAudioManager().update() between interaction.update() and tweens.update()', () => {
        ...
        const mixerMock = {
            update: jest.fn(() => { callOrder.push('mixer'); }),
        ...
        jest.doMock('@/input/InteractionManager', () => ({
            InteractionManager: jest.fn(() => ({
                update: jest.fn(() => { callOrder.push('interaction'); }),
                destroy: jest.fn(),
            })),
        }));
        ...
        const tweens = {
            update: jest.fn(() => { callOrder.push('tweens'); }),
        };
