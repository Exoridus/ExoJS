...
class TestWorkletFilter extends WorkletFilter {
    protected get _workletName(): string { return 'test-worklet'; }
    protected get _workletSource(): string { return '/* test */'; }
}
...
describe('WorkletFilter', () => {
    ...
    it('destroy disconnects all nodes', () => {
        const ctx = getAudioContext();
        const nodes: Array<{ disconnect: jest.Mock }> = [];
        const originalCreateGain = ctx.createGain.bind(ctx);
        jest.spyOn(ctx, 'createGain').mockImplementation(() => {
            const node = originalCreateGain() as unknown as { connect: jest.Mock; disconnect: jest.Mock; gain: unknown };
            node.connect = jest.fn();
    ...
    it('registration is cached: addModule not called again for same context+name after first load', async () => {
        ...
        class CacheTestFilter extends TestWorkletFilter {
            protected override get _workletName(): string { return uniqueName; }
        }
