...
describe('ChorusFilter', () => {
    ...
    describe('internal node graph wiring', () => {
        ...
        beforeEach(() => {
            ...
            jest.spyOn(ctx, 'createDelay').mockReturnValue(
                delayNode as unknown as DelayNode,
            );
            jest.spyOn(ctx, 'createOscillator').mockReturnValue(
                lfoOscillator as unknown as OscillatorNode,
            );
        });
    ...
    describe('delayMs setter', () => {
        it('updates delayNode.delayTime via setTargetAtTime', () => {
            ...
            filter.delayMs = 20;
            expect(filter.delayMs).toBe(20);
            expect(delayNode.delayTime.setTargetAtTime).toHaveBeenCalledWith(
                0.02, expect.anything(), expect.anything(),
            );
            filter.destroy();
        });
    ...
    describe('rateHz setter', () => {
        it('updates lfoOscillator.frequency via setTargetAtTime', () => {
            ...
            expect(lfoOscillator.frequency.setTargetAtTime).toHaveBeenCalledWith(
                3, expect.anything(), expect.anything(),
            );
            filter.destroy();
        });
    ...
    describe('wet setter', () => {
        it('updates dryGain (1-wet) and wetGain (wet) via setTargetAtTime', () => {
            ...
            let gainCallCount = 0;
            const gainNodes = [
                makeGainNode(ctx), makeGainNode(ctx),
                makeGainNode(ctx), makeGainNode(ctx), makeGainNode(ctx),
            ];
            jest.spyOn(ctx, 'createGain').mockImplementation(() => {
            ...
            expect(dryGain.gain.setTargetAtTime).toHaveBeenCalledWith(
                expect.closeTo(0.2, 5), expect.anything(), expect.anything(),
            );
            filter.destroy();
        });
