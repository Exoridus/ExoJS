 ...
 * onto InputSystem's own global frame journal, driven by real `PointerEvent`
 * dispatches through a real `<canvas>` via `BrowserPlatform`, into a real
 * `InputSystem` - not direct `GestureRecognizer` construction (see
...
// ---------------------------------------------------------------------------
// Helpers (mirrors test/input/pointer-channels.test.ts / input-system-events.test.ts)
// ---------------------------------------------------------------------------
...
/** Stand-in for `app.scenes`, holding the one flag `InputSystem` reads off it. */
...
const createInputSystem = (canvas?: HTMLCanvasElement): { im: InputSystem; canvas: HTMLCanvasElement; scenes: MockScenes } => {
  ...
  return { im: new InputSystem(createMockApp(c, scenes)), canvas: c, scenes };
};
...
/**
 * Run `milliseconds` of engine time through the system in 16 ms frames, the
...
const settleTwoTouchBaseline = (im: InputSystem, canvas: HTMLCanvasElement, bx: number, by: number): void => {
...
describe('InputSystem — gesture journal ordering', () => {
...
describe('InputSystem — long-press queuing', () => {
...
describe('InputSystem — destroy clears gesture holds and queued state', () => {
  test('destroy() before a pending long-press completes leaves it permanently unfired', () => {
    ...
    // A destroyed system is not driven any more; even if something still
...
describe('InputSystem — gesture center coordinates across multiple queued gestures in one frame', () => {
