...
const tick = (h: Harness): void => {
  h.input.preFrame(Time.seconds(0));

  try {
    h.interaction.preFrame(frameDelta);
  } finally {
...
describe('InputSystem — context-menu ordering and queuing (Bug B)', () => {
  ...
  test('a context-menu request attributed to a pointer whose terminal Leave arrived earlier the SAME flush still receives a live, not-yet-retired Pointer (invariants 6 & 7)', () => {
    ...
    // dispatch in ITS pass, which runs strictly after this input.preFrame().
...
describe('InteractionSystem — cross-pointer order preserved end-to-end (Bug C)', () => {
  ...
  test('a throwing node handler discards the failed interaction batch and leaves no stale state to replay', () => {
    ...
    input.preFrame(Time.seconds(0));
    expect(() => interaction.preFrame(frameDelta)).toThrow('expected handler failure');
    input._finishInteractionFrame();
