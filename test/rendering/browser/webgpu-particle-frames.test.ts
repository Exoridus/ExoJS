/**
 * WebGPU ParticleSystem texture-frame browser tests - opt-in, capability-aware.
 *
 * Which rect the compute path samples is something only a real device can
 * settle. Frame UVs reach the shader as per-instance `uvMin`/`uvMax`, and on
 * the GPU path those are packed by the compute shader from a uniform block
 * written once when the state is built - so a `textureFrame` chosen at any
 * other moment has to reach that block, or the two backends draw different
 * pixels from the same scene.
 *
