/**
 * What a render into a caller's own target leaves behind, on WebGL2.
 *
...
describe('a render into a caller-owned target (WebGL2)', () => {
  test('leaves the frame that follows it drawing what it drew before', async () => {
    ...
    const frame = (): number => {
      ...
      return readWebGl2Pixel(backend, size / 2, size / 2)[0]!;
    };
