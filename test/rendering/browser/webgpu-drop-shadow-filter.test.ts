...
describe('DropShadowFilter (WebGPU)', () => {
  test('draws the offset silhouette in the shadow colour behind the untouched source', async ctx => {
    await withScene(ctx, [dropShadow()], pixel => {
  ...
  test('shadowOnly leaves the source out', async ctx => {
    await withScene(ctx, [dropShadow({ shadowOnly: true })], pixel => {
  ...
  test('blur softens the shadow beyond its hard edge', async ctx => {
    await withScene(ctx, [dropShadow({ blur: 4, quality: 3 })], pixel => {
