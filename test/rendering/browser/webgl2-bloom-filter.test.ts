...
const withPlateau = async (levels: number, read: (pixel: (x: number, y: number) => RgbaTuple) => void): Promise<void> => {
  const backend = await createWebGl2TestBackend(BLUR_SCENE_SIZE);
...
const withBackdrop = async (read: (pixel: (x: number, y: number) => RgbaTuple) => void): Promise<void> => {
  const backend = await createWebGl2TestBackend(BLUR_SCENE_SIZE);
...
describe('BloomFilter (WebGL2)', () => {
  ...
  test('the glow inside a uniform bright region does not scale with the level count', async () => {
    let shallow = 0;
    let deep = 0;

    await withPlateau(1, pixel => {
      shallow = pixel(...PLATEAU)[0]!;
    });
    await withPlateau(3, pixel => {
  ...
  test('adds the glow as light, leaving the backdrop and the subject alpha alone', async () => {
    await withBackdrop(pixel => {
