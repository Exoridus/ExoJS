/**
 * What a surface gives back under the transport walk, on WebGPU - the same
 * scenes and the same expectations the WebGL2 spec runs.
 *
...
const pixel = (live: BounceHost, at: readonly [number, number]): readonly [number, number, number] => {
  const read = readWebGpuPixels(live.backend as WebGpuBackend, BOUNCE_SIZE)(at[0], at[1]);
...
describe('what a surface gives back (WebGPU)', () => {
