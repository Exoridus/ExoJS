...
describe('GlyphAtlasPool identity', () => {
  test('keys the atlas on the pixel ratio', () => {
    ...
    const one = pool.getAtlas('Roboto', 'normal', 'normal', '400', 'sdf', 8, 1);
    const two = pool.getAtlas('Roboto', 'normal', 'normal', '400', 'sdf', 8, 2);
    const three = pool.getAtlas('Roboto', 'normal', 'normal', '400', 'sdf', 8, 3);
  ...
  test('returns the same atlas for a repeated request', () => {
    ...
    expect(pool.getAtlas('Roboto', 'normal', 'normal', '400', 'sdf', 8, 2)).toBe(pool.getAtlas('Roboto', 'normal', 'normal', '400', 'sdf', 8, 2));
  });

  test('defaults to ratio 1', () => {
    const pool = new GlyphAtlasPool();

    expect(pool.getAtlas('Roboto', 'normal', 'normal', '400').pixelRatio).toBe(1);
    expect(pool.getAtlas('Roboto', 'normal', 'normal', '400')).toBe(pool.getAtlas('Roboto', 'normal', 'normal', '400', 'sdf', 8, 1));
  });
  ...
  test('shares one metrics instance across every ratio, mode and radius of a variant', () => {
    ...
    const metrics = pool.getMetrics('Roboto', 'normal', 'normal', '400');

    expect(pool.getAtlas('Roboto', 'normal', 'normal', '400', 'sdf', 8, 1).metrics).toBe(metrics);
    expect(pool.getAtlas('Roboto', 'normal', 'normal', '400', 'sdf', 8, 3).metrics).toBe(metrics);
    expect(pool.getAtlas('Roboto', 'normal', 'normal', '400', 'color', 8, 2).metrics).toBe(metrics);
    expect(pool.getAtlas('Roboto', 'normal', 'normal', '400', 'sdf', 16, 2).metrics).toBe(metrics);
  });

  test('separates metrics per font variant', () => {
    const pool = new GlyphAtlasPool();

    expect(pool.getMetrics('Roboto', 'normal', 'normal', '400')).not.toBe(pool.getMetrics('Roboto', 'italic', 'normal', '400'));
    expect(pool.getMetrics('Roboto', 'normal', 'normal', '400')).not.toBe(pool.getMetrics('Roboto', 'normal', 'normal', '700'));
    expect(pool.getMetrics('Roboto', 'normal', 'normal', '400')).not.toBe(pool.getMetrics('Inter', 'normal', 'normal', '400'));
  });
  ...
  test('keys the atlas and its metrics on the caps variant', () => {
    ...
    expect(pool.getAtlas('Roboto', 'normal', 'normal', '400')).not.toBe(pool.getAtlas('Roboto', 'normal', 'small-caps', '400'));
    expect(pool.getMetrics('Roboto', 'normal', 'normal', '400')).not.toBe(pool.getMetrics('Roboto', 'normal', 'small-caps', '400'));
  });

  test('keys the atlas on oblique separately from italic', () => {
    const pool = new GlyphAtlasPool();

    expect(pool.getAtlas('Roboto', 'italic', 'normal', '400')).not.toBe(pool.getAtlas('Roboto', 'oblique', 'normal', '400'));
  });

  test('clearing a typeface reaches every caps variant of it', () => {
    const pool = new GlyphAtlasPool();
    const upright = pool.getAtlas('Roboto', 'normal', 'normal', '400');
    const smallCaps = pool.getAtlas('Roboto', 'normal', 'small-caps', '400');
    const cleared: string[] = [];

    upright.onCleared.add(() => cleared.push('upright'));
    smallCaps.onCleared.add(() => cleared.push('small-caps'));

    pool.clearVariant('Roboto', 'normal', '400');
...
describe('GlyphAtlas rasterization at a pixel ratio', () => {
  const atlasAt = (pixelRatio: number): GlyphAtlas => new GlyphAtlas('Roboto', 'normal', 'normal', '400', 1024, 'color', 8, pixelRatio);
