...
const requestedRatios = (): number[] => getAtlas.mock.calls.map(call => (call as unknown as unknown[])[6] as number);
...
describe('Text atlas acquisition', () => {
  ...
  test('applies to colour glyphs too', () => {
    ...
    expect(getAtlas).toHaveBeenCalledWith('Arial', 'normal', 'normal', 'normal', 'color', 8, 3);
  });
