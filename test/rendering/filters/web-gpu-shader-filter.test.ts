...
interface MockWebGpuEnv {
    ...
    readonly buffers: Array<{ destroy: jest.Mock; size: number; }>;
    restore(): void;
}
...
function createMockWebGpuEnv(): MockWebGpuEnv {
    ...
    const buffers: Array<{ destroy: jest.Mock; size: number; }> = [];
...
describe('WebGpuShaderFilter', () => {
    ...
    test('throws when constructed without fragmentSource', () => {
        expect(() => new WebGpuShaderFilter({} as WebGpuShaderFilterOptions)).toThrow(
            'WebGpuShaderFilter requires fragmentSource.',
        );
    });
    ...
    test('vec3 uniform occupies a 16-byte aligned slot (not 12-byte)', () => {
        ...
        const userUboCall = env.queue.writeBuffer.mock.calls.find((args: unknown[]) => {
            ...
            return data instanceof Float32Array && data.length === 8
    ...
    test('vec4 uniform occupies a 16-byte aligned slot', () => {
        ...
        const userUboCall = env.queue.writeBuffer.mock.calls.find((args: unknown[]) => {
            ...
            return data instanceof Float32Array && data.length === 4
                && Math.abs(data[0] - 0.1) < 0.001 && Math.abs(data[3] - 0.4) < 0.001;
        });
    ...
    test('render pipeline is created with triangle-strip topology', () => {
        ...
        expect((pipelineDesc.primitive as GPUPrimitiveState).topology).toBe('triangle-strip');
