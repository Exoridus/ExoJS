...
describe('ParticleSystem SoA storage', () => {
    ...
    test('rejects non-positive capacity', () => {
        ...
        expect(() => new ParticleSystem(tex, 0)).toThrow();
        expect(() => new ParticleSystem(tex, -1)).toThrow();
        expect(() => new ParticleSystem(tex, 1.5)).toThrow();
    });

    test('spawn returns sequential slots up to capacity', () => {
        const system = new ParticleSystem(makeTexture(), 3);
    ...
    test('compaction removes expired particles in update()', () => {
        const system = new ParticleSystem(makeTexture(), 8);
    ...
    test('integration advances position by velocity * dt', () => {
        const system = new ParticleSystem(makeTexture(), 4);
        const slot = system.spawn();
...
describe('SpawnModule', () => {
    ...
    test('BurstSpawn fires at scheduled times', () => {
        const system = new ParticleSystem(makeTexture(), 128);
...
describe('DeathModule', () => {
    test('SpawnOnDeath fires when particle expires and copies position', () => {
        const parent = new ParticleSystem(makeTexture(), 4);
        const child = new ParticleSystem(makeTexture(), 16);
