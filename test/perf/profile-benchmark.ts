...
import { CallCounter, MemoryTracker, SubTimingTracker } from './profile-runner';
...
const scenarioResults: ProfileScenarioResult[] = [];
...
const deriveWins = (results: ProfileScenarioResult[]): Win[] => {
  ...
  for (let rank = 0; rank < Math.min(3, scored.length); rank++) {
    ...
    const hotPct = hotPhase && r.totalMs > 0 ? ((hotPhase.totalMs / r.totalMs) * 100).toFixed(0) : '?';
    ...
    if (r.name.startsWith('sat-polygon')) {
      const getNormalsTime = r.subTimings.filter(t => t.label.startsWith('getNormals')).reduce((s, t) => s + t.totalMs, 0);
      const getNormalsPct = r.totalMs > 0 ? ((getNormalsTime / r.totalMs) * 100).toFixed(0) : '?';
      observation =
        `getNormals() (both shapes) consumes ~${getNormalsPct}% of total SAT time. ` +
      ...
      recommendation =
        'Cache getNormals() result on Polygon (invalidate on point mutation). ' +
      ...
      observation =
        `All ${r.callCounts.find(c => c.label === 'leaf-tests')?.count ?? 0} leaf-tests ` +
      ...
      recommendation =
        'Pass a pre-allocated result buffer into queryPoint() on hot paths to eliminate ' +
      ...
      observation =
        `Linear tree-walk averages ${walkAvg.toFixed(4)} ms/query over ${QUERIES_PER_FRAME_REF} queries/frame. ` +
      ...
      observation =
        `Full 11k-node subtree invalidation triggered on every setPosition() call ` +
      ...
      recommendation =
        'Batch / coalesce transform invalidations within a frame — only flush dirty subtrees ' +
      ...
      observation =
        `${playCalls} play() calls total; avg ${playAvg.toFixed(5)} ms/call. ` +
        `Memory delta ${heapMb.toFixed(2)} MB — buffer-source nodes allocated per play().`;
      recommendation =
        'Expand pool size or introduce source-node recycling. ' +
...
const fmtTable = (rows: string[][], headers: string[]): string => {
  const all = [headers, ...rows];
  const widths = headers.map((_, ci) => Math.max(...all.map(row => (row[ci] ?? '').length)));
...
const md = [
  ...
  '',
  ...wins.map(w => [`${w.rank}. **${w.scenario}** — ${w.observation}`, `   - Recommendation: ${w.recommendation}`].join('\n')),
  '',
  '---',
  '',
