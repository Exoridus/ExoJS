    ...
    results.push(runScenario({
        ...
        setup() {
            for (let i = 0; i < PAIRS; i++) {
                ...
                polygons.push([
                    makeRegularPolygon(ax, ay, 20 + rand() * 30, sidesA),
                    makeRegularPolygon(bx, by, 20 + rand() * 30, sidesB),
                ]);
            }
        },
        tick() {
...

{
    ...
    results.push(runScenario({
        ...
        teardown() {
            for (const [a, b] of circles) { a.destroy(); b.destroy(); }
            circles.length = 0;
        },
    }, 30));
}
...

{
    ...
    results.push(runScenario({
        ...
        teardown() {
            for (const item of items) { item.bounds.destroy(); }
            items = [];
            bounds.destroy();
        },
    }, 120));
