...
export abstract class Joint {
  ...
  /**
   * @internal - whether this joint solves this frame. Set in {@link _prepare}
   * (disabled, sleeping or two static bodies → `false`), and read by the world
   * right after it to collect the joints the sub-steps actually have to touch.
   * A subclass may still clear it inside `_prepare` once it knows there is
   * nothing to solve (a slack rope limit, say).
   */
