...
Reference documentation for `@codexo/exojs-bench`: what every scenario measures,
what each metric means, what the harness pins about the browser and the machine,
and what the published comparison is allowed to claim.

Start at the [package README](../README.md) if you only want to run a benchmark.
Everything below is the contract behind the numbers it prints.
...
`filter-chain-4`, `mask-clip`, `mask-clip-animated`, `composite`. Each pins nesting depth, texture count, per-frame
...
only in re-setting strings; `mask-clip-animated` differs from `mask-clip` only in
moving its mask rects every frame, so the delta is what an effect change alone
costs the retained products around it; each `filter-chain-*` step adds one render-target
