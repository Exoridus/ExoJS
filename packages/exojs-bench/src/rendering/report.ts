...
const toMarkdown = (data: ReportData): string => {
  ...
  lines.push(
    ...
    '- **Phaser runs on a supplied WebGL2 context (not its default).** Phaser 4.2 is often described as a from-scratch WebGL2 renderer; empirically it is not. Its `WebGLRenderer` requests a WebGL**1** context by default (`canvas.getContext(\'webgl\')`), uses GLSL ES 1.00 shaders, and polyfills the WebGL2-core features it needs (instanced arrays, VAO) from WebGL1 extensions — its renderer is an evolution of the Phaser 3.85+ WebGL path, not a WebGL2 rewrite. To honour the `webgl2` backend contract (every arm renders through a WebGL2 context so the draw-call probe attaches and the comparison is apples-to-apples), the Phaser arm creates a WebGL2 context itself and hands it to Phaser via the documented `context` game-config option; Phaser\'s renderer supports a non-WebGL1 context and drives native `gl.drawElements`/`gl.drawArraysInstanced` on it, so the structural probe attaches and the Phaser arm reports **real** structural counters like every other WebGL2 arm — none are faked. Caveat to weigh: this measures Phaser through WebGL2, which is not its default (a stock Phaser 4 app renders WebGL1); the renderer code is otherwise its normal path (same GLSL 1.00 program, same batcher), only the context type differs. Phaser 4 ships no WebGPU renderer, so it never runs the `webgpu` backend.',
