  ...
  try {
    ...
    const provenance: PhysicsProvenance = {
      ...
      caveats: [
        ...
        'The joint scene is NOT equivalent across the arms, and its cross-arm comparison is withheld for that reason. Each link is pinned at the edge it shares with the next, so whether that pair also registers a contact comes down to each library default for connected bodies and to its contact margin at a touching edge - and they disagree: exojs reports one contact per jointed pair, rapier one per chain, and matter-js, planck and nape-js none. Every arm still runs the identical scene and its per-arm times are published, but the arms are not performing the same contact work, so no factor, winner or ranking is drawn from that row until one semantics is chosen and every arm is configured to it.',
