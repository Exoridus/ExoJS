 ...
 * @internal
 */
export class AnimatedAlphaOccluder implements OccluderSource {
  ...
  private _sync(): void {
    ...
    if (!(texture instanceof Texture)) {
      this._warnOnce(
        texture === null
          ? 'The drawable has no texture, so nothing was traced and it casts no shadow. Build the source once a texture is set.'
          : 'A RenderTexture has no pixels on this side of the GPU, so nothing was traced and the drawable casts no shadow. ' +
    ...
    if (traced === null) {
      ...
      this._warnOnce(
        'The texture could not be read, so nothing was traced and it casts no shadow yet. It is still loading, backed by nothing ' +
  ...
  private _warnOnce(message: string): void {
    if (__DEV__ && !this._warned) {
      this._warned = true;
      logger.warn(message, { source: 'AlphaOccluder' });
    }
  }
}
