...
sourceUrl: "https://github.com/Exoridus/ExoJS/blob/main/src/input/GamepadButton.ts"
---
## Import
...
- `transformValue(value: number): number` - Apply the button's transform pipeline to a raw browser button value (typically Gamepad.buttons[i].value, in 0..1). Pipeline: clamp to [0, 1] → optional invert → deadzone (returns 0 when the result is at or below threshold).
...
[src/input/GamepadButton.ts](https://github.com/Exoridus/ExoJS/blob/main/src/input/GamepadButton.ts)
