...
sourceUrl: "https://github.com/Exoridus/ExoJS/blob/main/src/input/GamepadAxis.ts"
---
## Import
...
- `transformValue(value: number): number` - Apply the axis transform pipeline to a raw browser axis value (typically Gamepad.axes[i], in -1..1). Pipeline: clamp to [-1, 1] → optional invert → optional normalize to [0, 1] → bipolar passthrough OR deadzone (returns 0 when the absolute value is at or below threshold).
...
[src/input/GamepadAxis.ts](https://github.com/Exoridus/ExoJS/blob/main/src/input/GamepadAxis.ts)
