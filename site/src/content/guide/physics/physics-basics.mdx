...
is a stateless sort-and-sweep: every fixed step, it re-sorts **all** live
colliders by their AABB's X position and sweeps once. That is exact (no missed
overlaps) and deterministic, and it is cheap enough for typical scenes — a
platformer level, a puzzle board, a few hundred physics props. It re-sorts
from scratch every step rather than keeping a persistent structure, so it
degrades as the live collider count grows into the thousands, and there is no
built-in spatial hash or incremental broadphase for very high-N worlds (tens
of thousands of simultaneously-live colliders). If you hit that ceiling,
splitting a large world into several smaller `PhysicsWorld` instances (e.g.
one per room, chunk or off-screen region) keeps each instance's live collider
count low.

<Callout type="info" title="No built-in spatial broadphase">
There is currently no persistent/incremental broad phase (spatial hash, grid, or similar) in `@codexo/exojs-physics` — see the tracking issue "physics: spatial broadphase for high-N worlds (P4f)" for the follow-up.
</Callout>
