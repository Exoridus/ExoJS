...
A body is pure simulation — it has no visual. To draw it, link it to a `Drawable` (such as a `Sprite`) with a [`PhysicsBinding`](/ExoJS/en/api/physics-binding/). After each `step`, the binding writes the body's **position and rotation** onto the node (the body's angle is radians; the node's rotation is set in degrees for you):
...
If you'd rather position things yourself, skip the binding and read the body's transform after each step — `body.x`, `body.y` (world position) and `body.angle` (radians):
