...
> **Documented limit.** CCD sweeps the body's **centre point**, which is ideal for small,
> point-like projectiles (bullets, pellets). A **large** fast body can still clip a corner,
> because only its centre is swept — for those, raise
> [`subStepCount`](/ExoJS/en/api/physics-world/) (or the step rate) so each step covers less
> distance, or thicken the geometry it must not pass. A full swept-shape time-of-impact for
> large bodies is a future enhancement.
