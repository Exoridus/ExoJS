...
- `onTrigger` for release-time taps: menu toggles or actions that deliberately wait for a short press to finish.
- `onActive`/`onStop` pair for held-state actions: movement, charge attacks, UI scrolling.
- `onStart` for immediate press-time actions: jump, shoot, inventory open, or screenshot.
...
- **`ChordAction`** and **`SequenceAction`** — several channels required at once, or in order. Covered in their own chapter, [Chords and sequences](/ExoJS/en/guide/input/chords-and-sequences/).
...
Continue with [Mouse and pointer](/ExoJS/en/guide/input/mouse-and-pointer/), which covers the unified pointer system — mouse, touch, and pen input with multi-touch slot management and gesture recognition.
