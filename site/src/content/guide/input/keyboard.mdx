...
ExoJS maps every physical key to a stable numeric channel via the [`Keyboard`](/ExoJS/en/api/keyboard/) enum. `Keyboard.Space` is channel 288, `Keyboard.A` is 321, `Keyboard.Left` is 293 — the mapping derives from browser key codes so it is consistent across layouts and operating systems. Keys only fire while the canvas has focus; when focus leaves, all held keys are released automatically.
...
`app.input` exposes two lower-level signals that fire on every key transition, regardless of the current scene:
...
The full list includes punctuation, brackets, and navigation keys. All values match the browser's legacy `KeyboardEvent.keyCode` map so the constants are stable regardless of keyboard layout.
