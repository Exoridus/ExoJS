...
Keys are aliases — names you choose. The application's `resourcePath` option prepends a base prefix to all relative URLs, so these keys resolve to e.g. `assets/image/hero.png`.
