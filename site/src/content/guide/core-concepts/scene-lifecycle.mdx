...
In between frames, when input arrives:

5. `handleInput(event)` — react to keyboard, pointer, or wheel events. Return `true` to consume the event so it doesn't propagate to scenes below.

When the scene is removed from the stack:

6. `async unload(loader)` — release scene-private assets that aren't shared with another active scene.
...
## handleInput: event-driven input

The `handleInput` hook is called between frames whenever an input event reaches the scene. Unlike `update`, it doesn't run every frame and doesn't carry a time delta:

```js
handleInput(event) {
    if (event.type === 'keyDown' && event.channel === Keyboard.Escape) {
        this.app.sceneManager.pushScene(new PauseScene());
        return true;
    }
}
```

Return `true` to consume the event; any other return value lets it propagate to scenes below in the stack (subject to those scenes' input modes).

For most projects, `this.inputs.onTrigger(...)` from inside `init` is more ergonomic than `handleInput`:
...
Bindings created on `this.inputs` are automatically disposed when the scene is destroyed. The `handleInput` hook is the lower-level escape hatch when the binding API isn't expressive enough.
