...
Most non-trivial projects have more than one scene. The application owns a scene manager (`app.sceneManager`) that drives the active stack. Three operations cover the common cases:
...
await app.sceneManager.pushScene(pause);
```

A parallel setting controls input routing (`scene.inputMode` — `'capture'`, `'passthrough'`, or `'transparent'`). A modal pause scene typically captures input so the underlying game doesn't react to keystrokes meant for the menu.
