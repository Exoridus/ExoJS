...
ExoJS keeps two coordinate systems in mind at once. Drawables are positioned in **world space** — the coordinate system you write game logic in. Input and view APIs use logical **canvas space**, also called design or screen space. A [`View`](/ExoJS/en/api/view/) sits between the two and handles the mapping.
...
Pointer events always arrive in logical canvas space. Once the camera has panned or zoomed, convert them to world space before hit-testing — otherwise a click lands on the wrong object. The world-vs-screen-coords example below shows the conversion.
</Callout>
...
<SourceSnippet
  source="examples/guides/coordinates-and-views/views.ts"
  region="fixed-view"
  language="ts"
  title="examples/guides/coordinates-and-views/views.ts"
/>
...
Pan and zoom a view, convert pointer coordinates, and place world-space markers.
