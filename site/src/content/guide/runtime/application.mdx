...
- `app.coroutines` — the [`CoroutineSystem`](/ExoJS/en/api/coroutine-system/) that spreads heavy work over frames.
...
Scenes are where actual game logic lives. Continue with [Scenes & lifecycle](/ExoJS/en/guide/runtime/scenes-and-lifecycle/), which covers what a scene is, how it differs from the application, and the order its hooks run in.
