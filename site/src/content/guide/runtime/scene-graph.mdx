...
<SourceSnippet
  source="examples/guides/scene-graph/layers.ts"
  region="add-children"
  language="ts"
  title="examples/guides/scene-graph/layers.ts"
/>
...
<Callout type="info" title="Add order is paint order">
Children paint in the order you add them — last added sits on top. For depth that changes at runtime (a character stepping behind a wall), set `zIndex` rather than reordering the child list.
</Callout>
...
declare const child: Container;
declare const a: Container;
declare const b: Container;
const index = 0;
...
<Callout type="pitfall" title="removeChild doesn't free the node">
...
```ts
import { Container, Rectangle } from '@codexo/exojs';

declare const sceneRoot: Container;
declare const world: Container;

const viewport = new Container();
viewport.mask = new Rectangle(0, 0, 400, 300);
viewport.addChild(world);
sceneRoot.addChild(viewport);
```
...
Press 1, 2, or 3 to bring a draggable sprite to the front and inspect its `zIndex` and hit target.
...
The next chapter, [Coordinates and views](/ExoJS/en/guide/runtime/coordinates-and-views/), covers world-vs-screen coordinates, the camera (`View`), and how to handle different canvas sizes without breaking layout assumptions.
