...
<SourceSnippet
  source="examples/guides/sprites/hero-scene.ts"
  region="hero-scene"
  language="ts"
  title="examples/guides/sprites/hero-scene.ts"
/>
...
Playing named spritesheet clips with `AnimatedSprite`.

<TryIt
  examples={['sprites-textures/sprite-basics', 'tweens-animation/frame-animation']}
  api={['sprite', 'spritesheet', 'texture']}
/>

## Where to go next

Continue with [Text](/ExoJS/en/guide/rendering/text/), which covers GPU-accelerated text rendering — font loading, styling, layout, and using text nodes in the scene graph.
