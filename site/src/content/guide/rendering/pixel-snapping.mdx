...
Both modes are resolved in the vertex shaders against the composed device grid, identically on WebGL2 and WebGPU, so they stay correct even inside a `RetainedContainer`, add no per-frame CPU cost, and keep a snapped drawable fully eligible for retained instruction recording. `PixelSnapMode.Position` is honoured by **every** drawable; `PixelSnapMode.Geometry` adds boundary snapping only for the types above and behaves like `PixelSnapMode.Position` on the rest.
...
- **One rounding policy.** Both modes round entirely in the vertex shaders (`floor(x + 0.5)`, the exact device-pixel nearest-rounding both backends agree on): `PixelSnapMode.Position` rounds the drawable's origin, and `PixelSnapMode.Geometry` additionally rounds each shared boundary to the device grid. There is no CPU snap path — the same nearest-device-pixel policy runs on WebGL2 and WebGPU alike.
...
<ExamplePreview chapter="sprites-textures" slug="texture-sampling" />

Compare texture sampling modes and render-only snapping against a fixed reference while changing zoom.
