...
`PixelSnapMode.Position` is honoured by **every** drawable — the origin is rounded in the vertex shader against the composed device grid, identically on WebGL2 and WebGPU, so it stays correct even inside a `RetainedContainer` and adds no per-frame CPU cost. `PixelSnapMode.Geometry` adds boundary snapping only for the types above; on other drawables it currently behaves like `PixelSnapMode.Position`.
...
- **One rounding policy.** `PixelSnapMode.Position` rounds the drawable's origin in the vertex shader (`floor(x + 0.5)`, the exact device-pixel nearest-rounding both backends agree on); `PixelSnapMode.Geometry`'s extra boundary snapping is still computed on the CPU during render-data preparation. Both use the same nearest-device-pixel policy.
