...
<Callout type="warning" title="WebGL2 needs an extension for this one">
...
A group of moving sprites feeds one live `RenderTexture` and one frozen snapshot. Click to recapture the snapshot after the source texture is ready.

<a id="mini-map"></a>
<ExamplePreview chapter="render-targets" slug="mini-map" />

A second view of the world rendered into a small `RenderTexture` and displayed through a circular mask in the corner.
