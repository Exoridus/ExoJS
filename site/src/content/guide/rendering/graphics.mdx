...
The pen commands above draw as you call them: each segment becomes its own mesh, which is why a pen-drawn outline cannot be filled and why its segments overlap at the corners instead of joining. [`GraphicsPath`](/ExoJS/en/api/graphics-path/) is the other half of the story — a path you assemble first and draw afterwards, as one shape:
...
<ExamplePreview chapter="geometry-graphics" slug="graphics-gradient" />

Solid and gradient-filled shapes on one canvas, with a gradient stroke around the ring.
...
<TryIt
  examples={['geometry-graphics/graphics-gradient', 'geometry-graphics/mesh-deformed-grid']}
  api={['graphics', 'color']}
/>

## Where to go next

Continue with [Sprites](/ExoJS/en/guide/rendering/sprites/), which covers the primary textured drawable — loading images, positioning, sizing, and the relationship between textures and sprites.
