...
**The value to want is usually the inherited one.** Sharpness peaks where one atlas texel lands on one device pixel, which is what inheriting gives you, and both directions away from that cost something: below it the field is stretched over more pixels than it describes, above it the extra texels are resampled away while the memory is spent anyway. The cost is roughly the square of the ratio — measured over an ASCII set at 9, 11 and 16px: 146k atlas texels at ratio 1, 582k at ratio 2 and 1.29M at ratio 3, enough to push that set from one 1024x1024 page to two.

That leaves two honest uses. Raise it when a node's **on-screen** density exceeds the surface ratio — a label scaled up at runtime, or one drawn through a zoomed camera, samples its atlas at more device pixels than the surface ratio suggests. Lower it to trade sharpness for atlas memory on text that is small, static or far away.
