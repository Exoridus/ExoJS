...
Small type is where a device's remaining density is still visible. `Text.pixelRatio` raises the glyph raster for one node alone, without making the whole surface — and its fill rate — denser:
...
The cost is roughly the square of the ratio. Measured over an ASCII set at 9, 11 and 16px: 146k atlas texels at ratio 1, 582k at ratio 2 and 1.29M at ratio 3 — enough to push that set from one 1024x1024 page to two. Raise it for the labels that need it, not for a whole UI.
