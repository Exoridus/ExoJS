...
2. **Tweens** — automated interpolation of numeric properties over time, with easing, delay, repeat, yoyo, chaining, and lifecycle callbacks. Driven by the application's `TweenSystem` (`app.tweens`).
...
<SourceSnippet
  source="examples/guides/animation/tween-scene.ts"
  region="tween-init"
  language="ts"
  title="examples/guides/animation/tween-scene.ts"
/>
...
```ts no-check -- listing of the Ease members, not runnable code
...
```ts
import { type Application, Ease, Sprite } from '@codexo/exojs';

declare const application: Application;
declare const sprite: Sprite;

const moveRight = application.tweens.create(sprite.position)
    .to({ x: 700 }, 1.0)
    .easing(Ease.cubicInOut);

const moveDown = application.tweens.create(sprite.position)
    .to({ y: 500 }, 0.8)
    .easing(Ease.cubicOut);

const fadeOut = application.tweens.create(sprite.tint)
    .to({ a: 0 }, 0.5);
...
const first = application.tweens.create(sprite.position)
    .to({ x: 700 }, 1.0)

const second = application.tweens.create(sprite.position)
    .to({ y: 500 }, 0.8);

const third = application.tweens.create(sprite.tint)
    .to({ a: 0 }, 0.5);
...
const fadeIn = application.tweens.create(sprite.tint)
    .to({ a: 1 }, 0.4);

const moveLeft = application.tweens.create(sprite.position)
    .to({ x: 200 }, 0.6)
    .easing(Ease.cubicOut);

const scaleUp = application.tweens.create(sprite.scale)
    .to({ x: 1.2, y: 1.2 }, 0.6)
    .easing(Ease.cubicOut);
...
application.tweens.createSequencer()
    .then(fadeIn)
    .wait(0.5)
...
```ts
import { AnimatedSprite, Rectangle, Texture } from '@codexo/exojs';

declare const spriteTexture: Texture;

const player = new AnimatedSprite(spriteTexture);
...
declare const player: AnimatedSprite;

player.addClip('idle', { frames: [new Rectangle(0, 32, 32, 32)], fps: 1 });              // repeat: -1 (default) — loops forever
player.addClip('jump', { frames: [new Rectangle(128, 0, 32, 32)], fps: 1, repeat: 1 });  // plays once
```
...
- **`frameDurations`** — a per-frame hold time in seconds, parallel to `frames`. When present it drives playback timing directly and `fps` is ignored while advancing frames (still accepted as a display-only average). Use it for clips with intentionally uneven pacing, such as an impact frame that lingers longer than the rest.
...
```ts
import { AnimatedSprite, Rectangle, Time } from '@codexo/exojs';

declare const player: AnimatedSprite;
declare const frame0: Rectangle;
declare const frame1: Rectangle;
declare const frame2: Rectangle;

player.addClip('hit', {
    frames: [frame0, frame1, frame2],
    frameDurations: [Time.seconds(0.08), Time.seconds(0.04), Time.seconds(0.2)],  // frame1 is a quick flash, frame2 lingers
...
Playback advances by itself. Once the sprite is part of the scene tree, the engine's `AnimationSystem` (`app.animations`) ticks every playing `AnimatedSprite` once per frame — there is nothing to call from your own `update`:

<SourceSnippet
  source="examples/guides/animation/tween-scene.ts"
  region="animated-sprite-scene"
  language="ts"
  title="examples/guides/animation/tween-scene.ts"
/>

A sprite you never add to the tree — one drawn immediate-mode via `context.render(sprite)` — has no owning application to reach, so drive it yourself with `update(deltaSeconds)`. The argument is **seconds**, the same unit as `Tween.update`:
...
```ts
import { AnimatedSprite, Spritesheet, type SpritesheetData, Texture } from '@codexo/exojs';

declare const spriteTexture: Texture;
declare const sheetData: SpritesheetData;

const sheet = new Spritesheet(spriteTexture, sheetData);
...
Tweens are driven by the `TweenSystem` on every frame via `app.tweens.update(delta)`, which happens automatically. You never call `update` on individual tweens unless you created them standalone (without a system).

Frame-loop animation — the `update(delta)` pattern where you read `delta` and multiply transforms — is the right choice for continuous motion that doesn't have a defined endpoint: character movement, physics, any state that evolves based on input or simulation. Tweens are the right choice for motion with a clear start and end: UI transitions, camera pans, scripted sequences, one-shot effects.
...
Click to interrupt a moving sprite or press Space to enable a yoyo round trip.

<a id="frame-animation"></a>
<ExamplePreview chapter="tweens-animation" slug="frame-animation" />

Named spritesheet clips played by `AnimatedSprite`, with automatic scene-tree advancement.

## Where to go next

Continue with [Render targets](/ExoJS/en/guide/rendering/render-targets/), which covers off-screen rendering — how to draw into a `RenderTexture` and reuse it as a sprite texture for compositing, caching, or post-processing.
