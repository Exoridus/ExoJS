...
- `pixelSnapMode` position snapping is resolved on the GPU against the composed world origin, so it stays correct inside a group; `geometry`-mode boundary snapping is still computed CPU-side and opts the group out of batch recording.
