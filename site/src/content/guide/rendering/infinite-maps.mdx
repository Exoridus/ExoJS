...
The same example lets you switch from synchronous sampling to worker-backed sampling and increase the cost while keeping the world seed fixed.
...
- The [Streamed Terrain](/ExoJS/en/playground/?example=tilemap/worker-streamed-terrain) example explores an endless value-noise world, rerolls the seed, and compares sync and worker providers under a tunable sample cost.
- The [Tiled Infinite Map Streaming](/ExoJS/en/playground/?example=tilemap/tiled-infinite-map) example streams a hand-authored Tiled map instead of procedural terrain — a free-flying camera with two independently chunked layers.
