...
Give the same `Shader` a `wgsl` body built on [`INSTANCE_TRANSFORM_WGSL`](/ExoJS/en/api/functions/), which exposes the same two helpers. Two differences follow from WGSL itself:
