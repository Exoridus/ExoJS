...
```ts
app.loader.onCacheError.add(error => {
...
```ts
import { AssetCache, CacheRoute, IndexedDbStore, MemoryCacheStore, NetworkFirstPolicy } from '@codexo/exojs';
...
The handler `context` routes fetches through the application's cache configuration:
