...
Ask for the complete bytes when you want ExoJS to own them:

```ts
async load() {
    const intro = await this.loader.load(Asset.type('video', 'video/intro.mp4', { download: true }));
}
```

`download: true` fetches the whole file through the loader's cache pipeline before building the element. That is what makes it cacheable and available offline, gives real byte progress, and it is always what a container (`.exoa`) entry does — container bytes are already owned by the application. The trade is that nothing plays until everything has arrived.

Whichever transport an asset arrives through, it is the same canonical asset: a source loaded over the network and the same source unpacked from a container resolve to one identity and one resident resource, never two. That is also the limit of `download: true` — it decides how the asset is built by the load that *materializes* it, so a load that joins an already-resident asset gets what is resident. Acquire it through the downloading load first when a consumer depends on the byte backing.
...
```ts
import { AssetType, jsonSourceCodec, SingleEntryLayout, type AssetFactory, type AssetSourceCodec } from '@codexo/exojs';
...
```ts
import { Application, AssetCache, CacheRoute, IndexedDbStore, MemoryCacheStore, NetworkFirstPolicy, type LoaderOptions } from '@codexo/exojs';
