...
A scope name is a diagnostic label, not a lookup key. Creating two scopes with the same name still creates two independent owners. Child scopes have independent claims but are torn down with their parent. Create them only while the parent is live; do not acquire new work after teardown.
