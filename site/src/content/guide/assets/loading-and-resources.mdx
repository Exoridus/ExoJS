...
const SharedAssets = Assets.from({ logo: 'sprites/logo.png' });
const ForestLocalAssets = Assets.from({ tree: 'sprites/tree.png' });

export const ForestAssets = Assets.compose(SharedAssets, ForestLocalAssets);

ForestAssets.logo; // Texture — the very same handle as SharedAssets.logo
ForestAssets.tree; // Texture
```

A composition **shares** its inputs' handles instead of copying them: `ForestAssets.logo === SharedAssets.logo`, so loading the composition heals the handles the shared catalog already handed out. It adds no ownership of its own — loading and releasing behave exactly as they would for the underlying keys.
...
```ts
// Assets.compose(): duplicate catalog key "logo" — two catalogs define it,
// use Assets.extend() to override it deliberately.
Assets.compose(SharedAssets, Assets.from({ logo: 'sprites/other.png' }));
```

The same catalog arriving twice along different paths — a diamond — is not a conflict and deduplicates:

```ts
const Left = Assets.compose(SharedAssets, Assets.from({ tree: 'sprites/tree.png' }));
const Right = Assets.compose(SharedAssets, Assets.from({ rock: 'sprites/rock.png' }));

Assets.compose(Left, Right); // { logo, tree, rock } — `logo` counted once
```
...
```ts
export const CustomizedAssets = Assets.extend(SharedAssets, {
    logo: 'sprites/forest-logo.png',   // deliberate override — SharedAssets keeps its own
    tree: 'sprites/tree.png',          // new key
});
```
...
`loader.release(...)` drops your claim on an asset; the object itself keeps its identity and heals back to `'loading'` if you `get()` the same source again. It accepts a handle, an `Asset` descriptor, a whole catalog, or a `(type, source)` pair:
...
Releasing only ever drops *your* claim. Assets loaded through `scene.loader` belong to that scene and are released when it ends, so releasing at app level can never pull an asset out from under a scene that still needs it. The payload is evicted once the last owner lets go.
