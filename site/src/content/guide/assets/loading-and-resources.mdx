...
The snippets on this page that come from a runnable example open with a `const app = this.app; if (app === null) …` guard. `Scene.app` is `null` until the scene is attached, and the examples are compiled under `strict`, so they narrow it once and reuse the local — shorter prose snippets elsewhere just write `this.app` directly.
...
<SourceSnippet source="src/resources/AssetStatus.ts" region="asset-status" title="The status contract" />
