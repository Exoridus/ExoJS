...
Types that aren't seamless — `AudioStream`, `Video`, `BmFont`, `FontAsset`, and custom types — have no placeholder to hand back, so there's nothing to `get()` speculatively. Load them by reference and hold onto the resolved value: `const music = await loader.load(AudioStream.of('audio/theme.ogg'))`. There is no `has()`/`peek()` guard — for a seamless handle read `.ready` / `.state`; for a non-seamless resource, `await` the load (or its returned handle's `.loaded` promise) before you use it.
</Callout>
...
export const MenuAssets = Assets.from({
    logo:  'image/logo.png',
    music: AudioStream.of('audio/theme.ogg'),
});

export const Level1Assets = Assets.from({
    tiles: 'image/level-1/tiles.png',
    map:   Json.of<{ spawn: [number, number] }>('data/level-1.json'),
});
```
...
A backgrounded leaf is boosted to fetch immediately if something `get()`s or foreground-`load()`s it before the queue reaches it — so there's nothing to guard against. Request it in the background early, then `await loader.load(Level2Assets)` when you actually need it and it resolves as soon as the in-flight fetch finishes. `loader.loadAll()` resolves once the background queue has fully drained.
...
export class HeightField {
    ...
    public static of(source: string): Asset<HeightField> {
        return new Asset({ type: 'heightField', source }) as unknown as Asset<HeightField>;
    }
}
