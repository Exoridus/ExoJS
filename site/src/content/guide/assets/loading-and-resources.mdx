...
A bare string only works for a _literal_ path — the type is inferred at compile time from the extension. When the path is computed at runtime, or the type can't be inferred from a path at all, use the canonical `Asset.type(...)` descriptor:
