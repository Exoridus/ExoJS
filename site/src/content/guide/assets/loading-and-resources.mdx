...
Whichever transport an asset arrives through, it is the same canonical asset: a source loaded over the network and the same source unpacked from a container resolve to one identity and one resident resource, never two.
