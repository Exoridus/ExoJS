...
```ts no-check -- shows the key types a catalog infers; member reads with no surrounding scope
...
```ts no-check -- continues the asset type above; heightFieldType comes from it
