...
```ts
import { Application, AssetCache, Connectivity, ConnectivityPolicyResolver, IndexedDbStore, type LoaderOptions } from '@codexo/exojs';

// Built first, so the same instance reaches both the cache and the application.
...
const loader: LoaderOptions = {
  ...
  cache: new AssetCache({
    ...
    policy: new ConnectivityPolicyResolver(connectivity),
  }),
};

const app = new Application({ connectivity, loader });
```

`app.connectivity` is that same object. The resolver is what ties it to the cache: it is asked, once per acquisition, which policy that acquisition runs under.
...
<Callout type="hint" title="Streaming media has to be asked">
Music and video are streamed by the browser by default: the element owns the transfer and never holds more than it is playing, so the loader acquires nothing and there is nothing to cache. Pass `download: true` to acquire it instead:
...
A downloaded or cached media asset is persisted as a `Blob`, not as bytes on the JavaScript heap - which is the difference between caching a large video and running out of memory trying. On a cache hit the element is pointed at an object URL over that blob, and the URL lives exactly as long as the resource does:
