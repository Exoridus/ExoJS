...
`.ldtk` files once you activate the extension.
...
Once the extension is active, load a `.ldtk` file with an explicit descriptor or by path (the
`.ldtk` extension is registered):
...
```ts no-check -- property tour of the loaded world; member reads with no surrounding scope
...
```ts no-check -- property tour of a level's properties bag; member reads with no surrounding scope
...
```ts no-check -- continues the load above; world and scene come from it
...
```ts no-check -- continues the load above; level comes from it
...
```ts no-check -- continues the load above; world comes from it
...
- **Streaming and spawning:** [Worlds and level streaming](/ExoJS/en/guide/assets/worlds-and-spawning/)
  covers per-level loading with `ldtkProject`, the world/neighbour model, and turning authored
  objects into game objects.
- **Sibling extension:** the [Tiled maps](/ExoJS/en/guide/assets/tiled-maps/) chapter covers the
  same activation model for the other map-editor adapter.
