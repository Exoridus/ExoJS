...
A [`MapWorld`](/ExoJS/en/api/map-world/) is metadata about *where* levels are — never their contents. Each `MapLevel` in it carries a stable id, world-space bounds, whether its payload is stored externally, and the levels the editor recorded as adjacent.
...
LDtk worlds have independent coordinate spaces, so a multi-world project yields one `MapWorld` per world (`project.worlds`), never one merged world. Merging them would make `bounds` and `getLevelsInBounds` report overlaps that do not exist.
</Callout>
...
[`loadLevel`](/ExoJS/en/api/map-world-runtime/) returns a `MapLevelRuntime`: one object that owns everything the load produced. There is no second handle to remember. `destroy()` tears the level down in the order that is always safe — spawned objects first (in reverse spawn order), then the map, then the asset scope last, because everything before it may still be reading a texture the scope keeps resident.
...
Loading a whole project at once with the `ldtkMap` asset type is unchanged and remains the right choice for a game small enough not to need any of this. See [LDtk levels](/ExoJS/en/guide/assets/ldtk/).
</Callout>
...
Level editors place objects; games need entities. A [`MapObjectSpawner`](/ExoJS/en/api/map-object-spawner/) is the bridge, and it is a plain object you construct — **not** a global registry:
...
`MapLevelRuntime.destroy()` destroys that `TileMap`. Return a map built for this load — never a shared one such as an entry of an eagerly converted `LdtkMap.levels`, or the runtime destroys an object other code still holds.
</Callout>
