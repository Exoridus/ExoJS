...
the `asepriteSheet` type once you activate the extension.
...
The package root is side-effect-free: importing it changes nothing on its own. You decide which
`Application` gets the extension, which is what lets two applications in one process differ.
...
`loader.load('sprites/hero.json')` will not route here. Always name the `'asepriteSheet'` type.
The handler fetches the JSON, validates it (throwing `AsepriteFormatError`
...
```ts no-check -- property tour of the loaded sheet; member reads with no surrounding scope
...
```ts no-check -- continues the load above; loader and Asset come from that snippet
...
- **API reference:** [`AsepriteSheet`](/ExoJS/en/api/aseprite-sheet/) documents `clips`, `slices`,
  ...
  [`Loader`](/ExoJS/en/api/loader/) documents `load(...)` and explicit `Asset.type(...)` descriptors.
