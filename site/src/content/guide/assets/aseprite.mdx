...
[Aseprite](https://www.aseprite.org/) is a popular pixel-art and animation editor. The official `@codexo/exojs-aseprite` extension adds an `asepriteSheet` asset type that loads Aseprite's JSON sheet export through the normal [`Loader`](/ExoJS/en/api/loader/) pipeline — fetching the JSON, resolving and loading the packed image as a `Texture`, and returning an [`AsepriteSheet`](/ExoJS/en/api/aseprite-sheet/) with a frame `Spritesheet` and one playable clip per Aseprite tag.
...
Like all extensions, the core ships nothing Aseprite-specific — an `Application` only understands the `asepriteSheet` type once you activate the extension.

## Activation

Pass `asepriteExtension` to `ApplicationOptions.extensions`. The extension is bound to exactly this `Application`:
...
The package root is side-effect-free: importing it changes nothing on its own. You decide which `Application` gets the extension, which is what lets two applications in one process differ.
...
This is the same add-only model the [Tiled maps](/ExoJS/en/guide/assets/tiled-maps/) and [Particles](/ExoJS/en/guide/effects/particles/) extensions use.

## Loading a sheet

<Callout type="pitfall" title="A bare path won't reach the Aseprite handler">
The Aseprite binding claims no file extension, so `loader.load('hero.json')` falls through to the generic JSON handler instead of parsing the sheet. Name the `'asepriteSheet'` type explicitly with `Asset.type(...)`.
</Callout>

Once the extension is active, load the JSON export with an explicit descriptor:

```ts
import { Asset } from '@codexo/exojs';

const sheet = await loader.load(Asset.type('asepriteSheet', 'sprites/hero.json'));
```

Unlike the Tiled adapter, the Aseprite binding does **not** claim a file extension — a bare `loader.load('sprites/hero.json')` will not route here. Always name the `'asepriteSheet'` type. The handler fetches the JSON, validates it (throwing `AsepriteFormatError` on a malformed document), resolves `meta.image` relative to the JSON source, sub-loads it as a `Texture` through the same loader, and returns a fully-parsed `AsepriteSheet`.

## Playing tag animations

`AsepriteSheet.createAnimatedSprite()` returns an [`AnimatedSprite`](/ExoJS/en/api/animated-sprite/) with every Aseprite tag pre-defined as a named, playable clip. Call `play(tag)` with a tag name to start it:

<SourceSnippet
  source="examples/guides/aseprite/hero-sheet.ts"
  region="aseprite-scene"
  language="ts"
  title="examples/guides/aseprite/hero-sheet.ts"
/>

Like any `AnimatedSprite`, playback advances on its own once the sprite is in the scene tree — the engine's `AnimationSystem` ticks it every frame. See the [Animation](/ExoJS/en/guide/rendering/animation/) guide for the full clip/playback API.
...
Aseprite frame tags carry a `direction`: `forward` (the default), `reverse`, `pingpong`, or `pingpong_reverse`. `AsepriteSheet.parse()` expands a tag's declared `from`→`to` range into the actual sequence of frame indices before building the clip, since `AnimatedSprite` itself always plays forward through whatever frame list it's given:
...
Aseprite frame tags can also carry a `repeat` field — a numeric string (`"1"`, `"2"`, …) set from the Tag Properties panel — specifying how many full cycles the tag plays before stopping. `AsepriteSheet.parse()` maps it directly onto [`AnimatedSpriteClipDefinition.repeat`](/ExoJS/en/api/animated-sprite/):
...
`AnimatedSprite.repeat` can be read back at any time and overridden per-call via `play(name, { repeat })` — see the [`AnimatedSprite`](/ExoJS/en/api/animated-sprite/) API reference for the full playback surface.
...
- **Package README:** [`@codexo/exojs-aseprite`](https://github.com/Exoridus/ExoJS/blob/main/packages/exojs-aseprite/README.md) — install, entry points, and the supported-format matrix.
- **API reference:** [`AsepriteSheet`](/ExoJS/en/api/aseprite-sheet/) documents `clips`, `slices`, `spritesheet`, and `createAnimatedSprite()`; [`AnimatedSprite`](/ExoJS/en/api/animated-sprite/) documents clip definition (`repeat`, `frameDurations`, `frameOffsets`) and playback; [`Loader`](/ExoJS/en/api/loader/) documents `load(...)` and explicit `Asset.type(...)` descriptors.
- **Guides:** [Sprites](/ExoJS/en/guide/rendering/sprites/) and [Loading & resources](/ExoJS/en/guide/assets/loading-and-resources/) cover the rendering and asset model the sheet plugs into.
