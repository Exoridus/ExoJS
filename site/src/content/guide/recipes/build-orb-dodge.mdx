...
This chapter builds a small, complete game from start to finish. You will see how several ExoJS concepts fit together in one project: a scene that runs every frame, input actions, `Graphics` primitives for all visuals, a HUD with live score and time, and a separate game-over scene that transitions back into the game on restart.
...
The player is a filled circle drawn with `Graphics`. A `VectorAction` combines keyboard and controller movement into one value, which the scene uses without checking the active device:
...
In `update`, normalize the action value so diagonal movement is not faster than cardinal movement, then clamp the position inside the canvas:
...
Orbs are stored in an array of `{ gfx, vx, vy, danger }` objects. Danger orbs are red stars; collectible orbs are green circles.
...
- **HUD overlay** — [HUD overlay](/ExoJS/en/guide/runtime/ui-and-widgets/) — add screen-fixed UI to the game scene through `scene.ui`
