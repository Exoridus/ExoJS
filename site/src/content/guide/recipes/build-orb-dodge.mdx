...
`GameOverScene` declares its activation-data shape (`GameOverData`) and receives the final score and time as the second argument to `setScene` — `init(data)` reads `data.score` / `data.time` directly, so there's no separate mutator method or intermediate state to keep in sync:
