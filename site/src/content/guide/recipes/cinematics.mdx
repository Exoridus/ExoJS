...
A cinematic is a scene. It owns the sequence. Tweens drive every timed element — camera pans, title reveals, character entrances, music fades. The cinematic scene pushes itself onto the stack with `input: 'capture'` so the player can't interfere, and pops itself when the sequence ends.
...
Push the cinematic scene with `input: 'capture'` to prevent the player from interacting during the cutscene:

```js
// From the game scene:
this.app.sceneManager.pushScene(new CinematicScene(), {
    mode: 'opaque',   // hide the game behind the cinematic
    input: 'capture', // consume all input events
});
```

`mode: 'opaque'` means the underlying game scene neither renders nor updates — a clean separation. When the cinematic ends, pop the scene:
