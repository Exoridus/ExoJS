...
Continue with [Gameplay collision](/ExoJS/en/guide/recipes/gameplay-collision/), which covers the shape primitives, SAT-based overlap response, and quadtree broad-phase that gameplay is built on — the last building block before the capstone game. To experiment with any pattern from the Guide, open the [Playground](/ExoJS/en/playground/) and edit the examples directly.
