...
A cinematic is a scene. It owns the sequence. Tweens drive every timed element — camera pans, title reveals, character entrances, music fades. Push the cinematic scene with `input: 'capture'` so the player can't interfere, and pop it when the sequence ends.
...
this.app.sceneManager.pushScene(new CinematicScene(), {
    ...
    input: 'capture', // consume scene-level handleInput events
});
```
