...
A cinematic is a scene. It owns the sequence. Tweens drive every timed element — camera pans, title reveals, character entrances, music fades. Navigate to the cinematic scene via `app.scenes.setScene(new CinematicScene())` to fully replace the game scene while the cutscene plays, and call `app.scenes.setScene(...)` again when the sequence ends to return to gameplay.
...
this.app.scenes.setScene(new CinematicScene());
```
...
this.app.tweens.create(this.barSize)
    ...
    .onComplete(() => {
        this.app.scenes.setScene(new GameScene(), { transition: { type: 'fade' } });
    })
    .start();
```
...
init(loader) {
    ...
    this.inputs.onTrigger(Keyboard.Space, () => {
        this.app.scenes.setScene(new GameScene());
    });
