...
class CinematicScene extends Scene {
    ...
    init(loader) {
        ...
        this.musicVoice = this.app.audio.play(loader.get(AudioStream, 'track'), { loop: true, volume: 0.2 });
...
this.app.scene.setScene(new CinematicScene());
```
...
this.app.tweens.create(this.barSize)
    ...
    .onComplete(() => {
        this.app.scene.setScene(new GameScene(), { transition: { type: 'fade' } });
    })
    .start();
```
