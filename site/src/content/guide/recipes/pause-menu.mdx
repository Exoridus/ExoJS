...
A pause menu can freeze the game, display a menu overlay, and resume cleanly when dismissed. In ExoJS, this is a scene-stack operation: push a `PauseScene` on top of the game scene with `'overlay'` mode (so the game remains visible) and `'capture'` input mode for scene-level `handleInput` routing.
...
- `input: 'capture'` — scene-level `handleInput` events stop at the pause scene. (`this.inputs` bindings are registered on `app.input`, so gate gameplay actions explicitly while paused.)
