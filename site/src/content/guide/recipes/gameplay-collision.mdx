...
if (response) {
    player.setPosition(
        player.x + response.projectionV.x,
        player.y + response.projectionV.y,
    );
}
```
...
```ts
import { Sprite, Sweep, type Time } from '@codexo/exojs';

declare const player: Sprite;
declare const wall: Sprite;
declare const velocityX: number;
declare const velocityY: number;
declare const frameTime: Time;

// In your update loop:
const playerBounds = player.getBounds();
const moveX = velocityX * frameTime.seconds;
const moveY = velocityY * frameTime.seconds;
...
if (hit !== null) {
    ...
    const slideX = (velocityX - dot * hit.normalX) * remaining * frameTime.seconds;
    const slideY = (velocityY - dot * hit.normalY) * remaining * frameTime.seconds;
