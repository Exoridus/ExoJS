...
Click to trigger a red flash on a sprite using `Drawable.tint` and a tween.

<a id="screen-shake-on-explosion"></a>
The same click also spawns particles and shakes the view.

## Where to go next

Continue with [UI patterns](/ExoJS/en/guide/recipes/ui-patterns/), which covers in-canvas UI construction — dialog systems, typewriter text, progress indicators, and when to use DOM instead.
