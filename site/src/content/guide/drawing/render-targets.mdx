...
The `updateSource()` method on `RenderTexture` signals the GPU backend that the texture contents have changed. This is called automatically by the backend when the render target is released — you do not need to call it manually after `setRenderTarget(null)`.
...
The `resize()` method changes the texture dimensions. `powerOfTwo` reports whether both dimensions are powers of two — relevant for some mipmap and tiling scenarios.
...
The next section, [Input](/ExoJS/en/guide/input/keyboard/), covers keyboard, mouse, pointer, and gamepad handling — the other half of making interactive scenes. If you want to extend the render-to-texture pattern into multi-pass filtering and screen-space effects, see [Post-processing](/ExoJS/en/guide/effects/post-processing/) in the Effects section.
