...
`Ease` provides 30 standard Penner easing functions. All accept normalized time `t` in [0, 1] and return an eased value that starts at 0 and ends at 1:
...
`chain()` returns the next tween, so you can build sequences in a single expression:

```js
this.app.tweens.create(sprite.position)
    .to({ x: 700 }, 1.0)
    .chain(
        this.app.tweens.create(sprite.position)
            .to({ y: 500 }, 0.8)
...
Tween interpolation is purely numeric. Attempting to tween a non-number property logs a warning and skips it. Vector properties must be accessed by their numeric fields (`sprite.position.x`, `sprite.position.y`). `Color`, `Vector`, and `Matrix` objects cannot be tweened directly — tween their individual number components instead.
...
player.defineClip('jump', { frames: [new Rectangle(128, 0, 32, 32)], fps: 0, loop: false });
```
...
Call `player.update(delta)` each frame. The delta can be a `Time` object (from the scene's `update`) or a number in seconds:
