...
update(delta) {
    ...
    for (let row = 0; row < 64; row++) {
        buf[this.col * 64 + row] = bands[row];
    }

    this.col = (this.col + 1) % 256;
    this.specTex.commit();

    // Shift the visible region so the newest column is at the right edge
    this.specSprite.setSize(256, 64);
}
```
...
class AudioReactiveScene extends Scene {
    ...
    update(delta) {
        ...
        for (let i = 0; i < bands.length; i++) {
            ...
            this.bars.fillColor = Color.fromHSL(hue, 0.8, 0.55);
        ...
        const bg = Color.fromHSL(240, 0.3, 0.1 + this.detector.pulse * 0.15);
...
- `justBeat` is `true` for at most one or two visual frames per beat (with the default 30ms window at 60fps). If your frame rate drops below ~30fps, you may miss a `justBeat` window. For critical beat-triggered effects, prefer the `onBeat` signal — it is dispatched per-beat and consumed on the main thread alongside the next update cycle.
...
The previous chapter, [Beat detection](/ExoJS/en/guide/audio/beat-detection/), covers tempo tracking, frequency bands, and working with `BeatDetector`'s event signals in detail. For color-oriented effects driven by audio — palette cycling, noise overlays, shader filters — see [Filters](/ExoJS/en/guide/effects/filters/). If you want to write a custom shader that samples a spectrogram `DataTexture`, the next section's chapter [Custom mesh shaders](/ExoJS/en/guide/effects/custom-mesh-shaders/) covers that.
