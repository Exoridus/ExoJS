...
Browsers require a user gesture before audio playback starts. ExoJS handles this automatically: the `AudioContext` is created on first use and resumed (un-suspended) on the first `mousedown`, `touchstart`, or `touchend` event that reaches the canvas. You call `music.play()` at any point — the engine defers actual playback until the context is unlocked.

The auto-unlock is transparent for most projects. If you need to know when the context is ready, import `isAudioContextReady` from `@codexo/exojs` and poll it.
...
// trackA is now silent (and stopped by default), trackB is at full volume
```
...
- `'lru'` — least-recently-used. Keeps the most recent sounds alive.
- `'priority'` — evicts the lowest-priority source. Assign `priority` per `play()` call.
...
```js
const voiceBus = app.audio.getBus('voice-over');
sound.bus = voiceBus;
```
...
```js
sound.defineSprite('impact', { start: 0.5, duration: 0.3 });
sound.defineSprite('whoosh', { start: 1.2, duration: 0.4 });
