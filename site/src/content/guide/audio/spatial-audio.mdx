...
Stereo pan is achieved through interaural time difference and attenuation, not a simple balance knob. The result is that a source placed to the listener's right sounds subtly different from one placed to the left at the same distance — this is the Web Audio panner's intended behavior and can't be disabled. If you need precise channel-level control without spatial effects, route through a bus with `pan` rather than using spatial positioning.
