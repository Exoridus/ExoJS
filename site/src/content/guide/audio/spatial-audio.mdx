...
A `Sound` you never give a `position` plays non-spatially — full volume in both channels, ignoring the listener. Assign a position only to sources that live in the world, and set a live voice's `position` back to `null` to drop it out of the pan field again.
</Callout>
