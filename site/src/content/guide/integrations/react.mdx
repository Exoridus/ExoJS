...
<Callout type="pitfall" title="Don't fight the sizing modes with inline style">
With the default `'fixed'` sizing mode the engine never touches the canvas CSS, so you may style it freely. The `'fill'`, `'fit'`, `'shrink'`, and `'letterbox'` modes manage `canvas.style` themselves — size the container and let them observe it, rather than setting your own inline `style`.
</Callout>
