...
Each filter in the chain costs one additional render pass per frame for that drawable. Three filters on a container means three extra passes. The [Render pipeline debugging](/ExoJS/en/guide/debugging/debugging-and-inspection/) chapter covers pass-count inspection and reduction strategies.
