...
<Callout type="hint" title="Bake static filtered subtrees with `cacheAsBitmap`">
Set `cacheAsBitmap = true` on a filtered subtree that doesn't change each frame: the chain runs once, then draws as a single cached texture instead of re-running every filter every frame.
</Callout>
