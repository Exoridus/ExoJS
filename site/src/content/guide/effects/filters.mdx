...
```js
import { BlurFilter, ColorFilter } from '@codexo/exojs';
...
Set `filters` to an empty array to clear the chain. Assigning a new array replaces the old one — you don't need to manually remove individual filters. A null or empty array means no extra render passes.
...
`LutFilter.fromImage(image)` wraps an `HTMLImageElement` or `HTMLCanvasElement` as a texture with the correct sampling parameters (nearest, clamp, no mip). `identityLut1D()` and `identityLut3D()` create no-op identity textures for testing. The `setLut()` method swaps textures at runtime — instant, no shader recompilation.
