...
The scene describes what emits and what blocks. `quality` decides how that becomes pixels, and nothing else changes between the two — the same lights, the same occluders.
...
| Extra passes            | none                               | two, a third with normals                           |
...
lighting.quality; // 'forward' | 'lightmap'
```

Name a renderer outright when you need a property only that one has — `'forward'` for normal maps on a `LitMaterial`, `'lightmap'` for shadows and an uncapped light count.
