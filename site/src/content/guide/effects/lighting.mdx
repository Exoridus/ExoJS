...
<Callout type="note" title="A lit surface is not a source yet">
  Light is transported from the emitters and occluded by the same field the shadows use, but a wall this renderer lit does not itself emit. There is no bounced colour yet. A `SunLight` has nowhere to emit from and is skipped; a `SpotLight` emits like a point.
</Callout>
...
<a id="normal-mapped-blocks"></a>
<ExamplePreview chapter="lighting" slug="normal-mapped-blocks" />

The scene deferred 2D lighting plugins are demonstrated with, posed here instead: authored normal maps on a floor and three blocks, a directional light, and a point light on the pointer. Each drawable is drawn once and says where its normals come from, so there is no second sprite in a normal layer and no parent group to assign.
