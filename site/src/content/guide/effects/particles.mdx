...
```js
import { ParticleSystem, Texture } from '@codexo/exojs';
...
| `Gradient(keys)` | Same as `Curve` but interpolates `Color` values |

`Curve` and `Gradient` are `LifetimeFunction<T>` rather than `Distribution<T>` — they're evaluated with a normalised lifetime `t` in 0..1, not sampled at spawn. They're typically used with update modules, not spawn modules.
