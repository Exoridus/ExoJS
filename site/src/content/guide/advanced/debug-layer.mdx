...
ExoJS ships six diagnostic layers in the `@codexo/exojs/debug` subpath. They render as overlays on top of your running scene — no scene-logic changes, no special draw-call instrumentation, no build flags. Enable one during development to see bounding boxes, frame timing, hit-test state, or filter-chain pass counts overlaid on the canvas.
