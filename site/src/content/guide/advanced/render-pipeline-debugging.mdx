...
`RenderPassInspectorLayer` extends `DebugLayer` (which extends `Scene`), but it is designed to be driven via `app.onFrame` — the same pattern used by `DebugOverlay` for the other debug layers. It is not pushed onto the scene stack. The inspector walks `sceneManager.scene?.root` each frame, so it sees whichever scene is currently active.
...
The ExoJS engine emits debug-group labels around filter and mesh-custom-shader passes so these tools display meaningful pass names rather than generic WebGL/WebGPU calls. Look for these labels in capture tools:
...
These labels are set via the backend's debug group API (`pushDebugGroup`/`popDebugGroup` in WebGPU, `pushGroupMarker`/`popGroupMarker` in WebGL2) and are visible in any tool that respects the debug group protocol.
