...
Pass-reduction strategies in order of impact:
1. Remove filters you don't need.
2. Set `cacheAsBitmap = true` on filtered containers that don't change every frame.
3. Consolidate filter chains — two `ColorFilter` instances cost two passes; one `WebGpuShaderFilter` can apply both in one pass.
4. Keep filtered drawables small. A 2048×2048 blur costs dramatically more than a 256×256 one.
