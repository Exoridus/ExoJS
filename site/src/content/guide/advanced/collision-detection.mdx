...
All shapes carry a `position` (`Vector`) and expose `collisionType` for dispatch. Each can be queried against any other via `intersectsWith` and `collidesWith`.
...
All shape-pair combinations are supported — rectangle-rectangle, circle-circle, polygon-circle, ellipse-rectangle, etc. Ellipse and polygon shapes are converted to polygon approximations for SAT-based tests (ellipses use configurable segment approximations).

## Penetration response: collidesWith

`shapeA.collidesWith(shapeB)` returns a [`CollisionResponse`](/ExoJS/en/api/collision-response/) or `null` if no overlap:
...
Collision response is supported for these shape pairs: rectangle-rectangle, circle-circle, circle-rectangle, ellipse-rectangle, ellipse-circle, polygon-circle. The full SAT solver handles generic `Collidable` pairs. Pairs not listed above (`collidesWith` returns `null`) can still be tested with `intersectsWith` for boolean overlap.
...
`SceneNode.intersectsWith` dispatches based on whether the node's rotation is axis-aligned (fast AABB path) or rotated (SAT path). `SceneNode.collidesWith` returns a response for AABB pairs and delegates to SAT for rotated pairs.
