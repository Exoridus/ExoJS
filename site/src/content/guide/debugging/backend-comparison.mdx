...
<Callout type="invalid" title="Pin a backend only when you truly must">
...
A [`Shader`](/ExoJS/en/api/shader-source/) accepts both `glsl: { vertex, fragment }` and `wgsl` source; wrap it in a [`MeshMaterial`](/ExoJS/en/api/mesh-material/) to bind uniforms and attach it to a `Mesh`. The renderer picks the appropriate language for the active backend. Provide both for cross-backend portability. [`Shader.detectUniformDrift()`](/ExoJS/en/api/shader-source/#methods) compares declared uniforms across languages for CI-style verification.
...
<Callout type="broken" title="Device loss reclaims your raw GPU resources">
