...
<Callout type="info" title="Leave your debug logs in">
Only `Error` survives a production build — `Debug`, `Info` and `Warning` are stripped out. So `logger.debug(...)` and `logger.info(...)` cost nothing shipped; there's no need to remove them or gate them behind your own flag.
</Callout>
