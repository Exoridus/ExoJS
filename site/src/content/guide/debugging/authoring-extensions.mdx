...
```ts no-check -- annotated Extension template; every field is a placeholder
...
The binding arrays cover contributions the engine knows how to materialise for you. `install(app)` covers the rest: an app-level `System` on `app.systems`, a subscription on an application signal, a debug overlay next to the canvas, a worker, an observer. It runs once per `Application`, as the **final** construction step — every core manager and every materialised binding already exists — and dependencies listed in `dependencies` are installed before their dependents.
