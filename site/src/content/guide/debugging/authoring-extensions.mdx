...
<Callout type="hint" title="Ask the application what it got">
`app.equipment` returns the frozen snapshot that was materialised at construction — the descriptors plus the renderer, asset, serializer and system bindings flattened out of them. Read it instead of reconstructing the set from your import graph.
</Callout>
