...
- **`assets`** — an `AssetBinding` maps an asset constructor, its `AssetDefinitions` type, and optional file suffixes to a loader-local `AssetHandler` factory. This is how `@codexo/exojs-tiled` teaches the `Loader` about `.tmj` files.
