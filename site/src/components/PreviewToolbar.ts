...
export class PreviewToolbar extends LitElement {
    ...
    public render(): ReturnType<LitElement['render']> {
        ...
        return html`
            <div class="meta">
                ...
                ${this.exampleTitle && hasDimensions ? html`<span class="separator separator--dim" aria-hidden="true">·</span>` : nothing}
                ${hasDimensions
                    ? html`
                          <span class="dimensions" aria-label="Preview canvas size">
                              <span class="dimensions__value">${this.canvasWidth}×${this.canvasHeight}</span>
                              ${zoomPercent !== null ? html`<span class="zoom" aria-label="Zoom">· ${zoomPercent}%</span>` : nothing}
                          </span>
                      `
                    : nothing}
            </div>
            <div class="actions">
                ${this.version ? html`<span class="version" title="ExoJS runtime version">@codexo/exojs@${this.version}</span>` : nothing}
                <button
                    class="reload"
                    type="button"
    ...
    private _onReload(): void {
        ...
        this.dispatchEvent(
            new CustomEvent('request-reload', {
                bubbles: true,
                composed: true,
            })
        );
