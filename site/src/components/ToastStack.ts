...
export class ToastStack extends LitElement {
    ...
    public render(): ReturnType<LitElement['render']> {
        ...
        return html` <div class="stack" aria-label="Notifications">${this._toasts.map(toast => this._renderToast(toast))}</div> `;
    }
    ...
    private _renderToast(toast: ToastMessage): ReturnType<LitElement['render']> {
        return html`
            <div class="toast" data-toast-id=${toast.id} role="status" aria-live="polite" aria-atomic="true">
                ...
                ${toast.action ? html` <button class="action" @click=${() => this._runAction(toast)}>${toast.action.label}</button> ` : nothing}
                <button class="close" aria-label="Dismiss" title="Dismiss" @click=${() => dismissToast(toast.id)}>
