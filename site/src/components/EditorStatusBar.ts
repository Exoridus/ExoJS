        ...
        return html`
            ...
            <div class="right">
                ${this.selectionLength > 0
                    ? html` <span class="chip chip--selection" aria-label="Selection length"> ${this.selectionLength} selected </span> `
                    : nothing}
                <span class="chip chip--cursor" aria-label=${`Cursor position, line ${this.line}, column ${this.column}`}
                    >Ln ${this.line}, Col ${this.column}</span
                >
                <span class="chip chip--encoding" aria-hidden="true">UTF-8 · LF</span>
                <span class="chip chip--dirty" data-dirty=${String(this.dirty)} aria-label=${this.dirty ? 'Modified' : 'Saved'}>
