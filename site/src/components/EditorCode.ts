...
export class EditorCode extends LitElement {
                    ...
                    ${this._showMenu
                        ? html`
                              <div class="menu-dropdown" role="menu">
                                  ...
                                  <button class="menu-item" role="menuitem" data-variant="danger" ?disabled=${!this.canReset} @click=${this._resetCode}>
                                      Reset Code
                                  </button>
                              </div>
                          `
    ...
    public override connectedCallback(): void {
        ...
        if ((root instanceof ShadowRoot || root instanceof Document) && !root.querySelector('style[data-exo-code-editor-styles]')) {
    ...
    private async _ensureEditorState(sourceCode: string, sourcePath: string | null): Promise<void> {
        ...
        if (this.editorView === null) {
            ...
            await Promise.all([document.fonts.load('400 14px "JetBrains Mono"'), document.fonts.load('500 14px "JetBrains Mono"')]);
    ...
    private _initializeEditor(sourceCode: string, sourcePath: string | null): void {
        ...
        this._cursorListener = this.editorView.onDidChangeCursorSelection(event => {
            ...
            const selectionLength = selection.isEmpty() || !this._editorModel ? 0 : this._editorModel.getValueInRange(selection).length;
        ...
        // Monaco's horizontalDistanceToText calculation for this container setup).
        {
            ...
            this._hoverMouseListeners.push(
                this.editorView.onMouseMove(e => {
                    ...
                    const key = word ? `${pos.lineNumber}:${word.startColumn}-${word.endColumn}` : `${pos.lineNumber}:${pos.column}`;
                    if (key === shownForKey) {
        ...
        _showInlineErrorCommandId =
            this.editorView.addCommand(0, () => {
    ...
    private async _loadTypingsForVersion(versionId: string): Promise<ReadonlyArray<ExtraLib>> {
        const [shared, exojs] = await Promise.all([this._loadSharedTypings(), this._loadVersionedExoJsTypings(versionId)]);
        return [...shared, ...exojs];
    }
    ...
    private _dispatchDiagnostics(diagnostics: ReadonlyArray<EditorDiagnostic>): void {
        this.dispatchEvent(
            new CustomEvent<EditorDiagnosticEvent>('editor-diagnostic', {
                detail: { diagnostics },
                bubbles: true,
                composed: true,
            })
    ...
    private _dispatchCursor(lineNumber: number, column: number, selectionLength: number): void {
        this.dispatchEvent(
            new CustomEvent<EditorCursorEvent>('editor-cursor', {
    ...
    private _showInlineErrorForLine(lineNumber: number): void {
        ...
        const markers = monaco.editor.getModelMarkers({ resource: this._editorModel.uri }).filter(m => m.startLineNumber === lineNumber && m.severity >= 4);
    ...
    private _pruneStaleInlineErrors(): void {
        ...
        const errorLines = new Set(
            monaco.editor
                .getModelMarkers({ resource: this._editorModel.uri })
    ...
    private _exportCode(): void {
        ...
        const code = this.editorView.getValue();
        const fileName = this.sourcePath ? (this.sourcePath.split('/').pop() ?? 'example.js') : 'example.js';
    ...
    private _setDirty(next: boolean): void {
        ...
        this.dispatchEvent(
            new CustomEvent<EditorDirtyEvent>('editor-dirty', {
                detail: { dirty: next },
                bubbles: true,
                composed: true,
            })
