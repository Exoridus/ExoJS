...
function writeWav(samples) {
    ...
    buf.writeUInt32LE(SAMPLE_RATE * CHANNELS * BITS / 8, o); o += 4;
    buf.writeUInt16LE(CHANNELS * BITS / 8, o); o += 2;
...
for (const loop of LOOPS) {
    ...
    execSync(
        `ffmpeg -y -i "${wavPath}" -c:a libvorbis -q:a 2 -ac 1 -ar ${SAMPLE_RATE} "${oggPath}"`,
        { stdio: 'pipe' },
    );
