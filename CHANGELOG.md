...
- `Graphics` fill and stroke now accept `Gradient` paint values via new `fillGradient` / `lineGradient` properties (alongside the existing solid `fillColor` / `lineColor`). A gradient paint is cloned on assignment, rasterized once to a `DataTexture` via `Gradient.toTexture()`, and sampled across each shape's local bounding box as a textured `Mesh` (white tint, no extra Drawable). Setting `fillColor` / `lineColor` reverts that channel to the solid path; the most recently assigned of the pair wins. Gradient textures are owned by the `Graphics` and released on `clear()` / `destroy()`. `LinearGradient` and `RadialGradient` fills are rendered and pixel-tested on **WebGL2**. The same path runs on **WebGPU** without validation errors (covered by a draw/validation test), but pixel-correct gradient sampling on WebGPU is pending a separate fix to the backend's `DataTexture` upload path and is tracked as a follow-up.
