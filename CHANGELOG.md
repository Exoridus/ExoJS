...
- `Graphics` fill and stroke styles are set through `fillStyle` / `strokeStyle`, each accepting a solid `Color`, a `Gradient`, or `null` (revert to the solid `fillColor` / `lineColor`). `fillColor` / `lineColor` remain as color-only convenience accessors over those styles: assigning either copies the color into its slot and makes it the active style, replacing any gradient. Each channel keeps a single active style internally (no parallel color-plus-gradient state). A `Gradient` style is cloned on assignment, rasterized once to a `DataTexture` via `Gradient.toTexture()`, and sampled across each shape's local bounding box as a textured `Mesh` (white tint, no extra Drawable / no `GradientDrawable`). Gradient textures are owned by the `Graphics` and released on `clear()` / `destroy()`. `LinearGradient` and `RadialGradient` fills are rendered and pixel-tested on **WebGL2** and **WebGPU**.
...
- WebGL2 text rendering now compiles on strict GLSL ES 3.00 drivers. `text-color.frag` declared a local variable named `sample`, which is a reserved word in GLSL ES 3.00; strict ANGLE/SwiftShader builds rejected the shader at compile time. Because the WebGL2 backend compiles its renderer shaders (including the text renderer) during initialization, the invalid shader aborted WebGL2 backend startup entirely on those drivers. The local was renamed to `texel` — no shader math, no output, and no public API change. A reserved-word audit of all WebGL2 GLSL sources found `sample` to be the only conflicting identifier.
- Added real-shader compile regression coverage (`test/rendering/browser/webgl2-shader-compile.test.ts`): it imports the engine's actual WebGL2 `.vert`/`.frag` sources via `?raw` — bypassing the vitest shader stub that rewrites shader imports to empty strings and had hidden this class of bug — then compiles every shader and links each renderer program pair on the WebGL2 (SwiftShader) browser driver. A reserved-word regression now fails the WebGL2 browser test suite.
...
- Promoted the previously-orphaned `rendering/camera-basic.js` into the catalog as `application-scenes/camera-basic` — a high-level `Camera` facade demo (pointer-follow camera, mouse-wheel zoom, slow world rotation, and a screen-space HUD bar drawn through a per-render view override). Resolves the orphan documented in the previous playground PR; complements the low-level `View` demo in `camera-and-view`.
- Fixed `text-fonts/basic-text.js` and `text-fonts/web-fonts.js` loading fonts with `loader.load(FontFace, …)`. `FontFace` is the browser global, but the loader registers its font factory under the `FontAsset` token (a font load resolves to a browser `FontFace`), so the wrong token threw `No factory registered for FontFace` at runtime. Both now import `FontAsset` and call `loader.load(FontAsset, …)`, matching the documented API.
- Fixed `spatial-audio/falloff-curves.js` calling `Graphics.drawRect(…)`, which is not a `Graphics` method — the rectangle primitive is `drawRectangle(…)`. The stale name threw `this._graphics.drawRect is not a function` on the first frame.
- Fixed `debug-layer/signal-bus-inspector.js` reading `signal.bindings.length`; `Signal` has no public `bindings` array and exposes the live listener total as `signal.count`, so the old access threw `Cannot read properties of undefined (reading 'length')`.
- These three runtime regressions were surfaced by re-running the example smoke harness (`pnpm test:examples:smoke`) now that the WebGL2 text-shader blocker is cleared.
- Fixed all five `text-fonts/*` examples using stale PixiJS-era `TextStyle` keys that the current API silently ignores at runtime, so the runtime smoke could not catch them. `fill` → `fillColor` (a `Color`, not a CSS string); `stroke` / `strokeThickness` → `outlineColor` / `outlineWidth` (SDF units 0..0.5); and word-wrap moved off the style object onto the third `Text` constructor argument (`LayoutOptions`): `wordWrap` / `wordWrapWidth` → `maxWidth`, with `breakWords` for character-level breaking. Affected: `basic-text.js`, `web-fonts.js`, `text-glitch.js`, plus rewrites of `multiline-and-wrap.js` (now demonstrates no-wrap, `maxWidth` word-wrap, and `breakWords` in stacked vertical sections instead of overlapping columns) and `stroke-and-shadow.js` (now uses the native SDF outline + drop-shadow style fields instead of stacking a second offset `Text`).
- The "Multiline And Wrap" example specifically rendered as overlapping, unreadable text because `wordWrap: true` / `wordWrapWidth` were ignored, so every paragraph laid out as a single overflowing line — and the editor flagged `fill` as an invalid `TextStyleOptions` property. Both are resolved; the example now wraps correctly and type-checks.
- Migrated the remaining playground examples off the stale PixiJS-era `TextStyle` keys that the current `Text` API silently ignores at runtime (37 examples across 13 categories: `audio-basics`, `audio-fx`, `spatial-audio`, `beat-detection`, `tweens-animation`, `scene-graph`, `application-scenes`, `showcase`, `particles`, `sprites-textures`, `input`, `getting-started`, `debug-layer`). `fill: 'white'` / `fill: '#fff'` → `fillColor: Color.white`; the ignored `padding` style key was removed (no current `TextStyleOptions` equivalent); and `showcase/typewriter-text.js` / `showcase/dialog-system.js` moved `wordWrap` / `wordWrapWidth` off the style object onto the third `Text` constructor argument as `LayoutOptions` (`{ maxWidth }`). Every migrated `fill` value was white, so no visual intent changed — but non-white/fancier styles would previously have failed silently, and the editor/type-checker correctly flagged the stale keys. No engine or rendering code was touched.

### Tests

- Added Text layout/style regression coverage that the runtime example smoke cannot provide. A compile-time API contract (`test/rendering/text/text-style-options.test.ts`) pins the canonical `TextStyleOptions` / `LayoutOptions` keys and `@ts-expect-error`s the stale `fill` / `stroke` / `wordWrap` aliases, so reintroducing them — in the API or by mirroring the option types — fails `tsc --noEmit`. New `layoutText` unit cases assert that an unbroken long token stays single-line without `breakWords` and wraps with it, and that a spaced string stays single-line when no `maxWidth` is set. A WebGL2 browser test (`test/rendering/browser/webgl2-text-layout.test.ts`) drives the real backend + real glyph atlas through the GPU draw path and asserts, on the produced glyph geometry, that letters are horizontally separated (non-overlapping), wrapped text splits across vertical line bands, and unbounded text stays on one line.
- Added `pnpm typecheck:examples` (`tsconfig.examples.json`): a lightweight `checkJs` pass that type-checks the example sources against the engine's own source types (no build required). It surfaces the editor-visible "`fill` does not exist in `TextStyleOptions`" diagnostics that the runtime smoke silently passes, catching stale Text style API in examples. It now covers the **full example catalog** (`examples/**/*.js`, ~98 sources) rather than only `text-fonts/*`, so the TextStyle migration is gated catalog-wide. Nineteen examples are listed in `exclude` because they currently fail the check for reasons unrelated to text styling (stale two-argument `Signal.add` / `Scene.setScene`, particle-module constructor arity, `Sprite` dynamic instance fields, `AudioAnalyser` API drift, WGSL module types); their TextStyle migration is already complete, and the remaining non-Text type drift is tracked as follow-up batches. `examples/shared/editor-support.d.ts` is intentionally not part of the program (it is a Monaco-only extra-lib whose `declare module '@codexo/exojs'` re-declarations collide with the real source exports under `tsc`).
...
- Added a standalone runtime smoke harness (`pnpm test:examples:smoke`) that loads every catalog example through the real `preview.html` import map in headless Chromium and asserts no uncaught error plus a mounted `<canvas>`, capability-gating WebGPU-only examples and writing a pass/fail matrix. It is a diagnostic script, intentionally not part of `npm test`. Its first run surfaced a single engine blocker — `src/rendering/webgl2/glsl/text-color.frag` declares a variable named `sample`, a reserved word in GLSL ES 3.00, which strict drivers (ANGLE/SwiftShader) reject during WebGL2 backend startup; the existing browser tests miss it because `vitest` stubs shader imports to empty strings. The shader fix is tracked as a separate engine-scoped change.
