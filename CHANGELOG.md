...
- Added `pnpm typecheck:examples` (`tsconfig.examples.json`): a lightweight `checkJs` pass that type-checks the `text-fonts/*` example sources against the engine's own source types (no build required). It surfaces the editor-visible "`fill` does not exist in `TextStyleOptions`" diagnostics that the runtime smoke silently passes, catching stale Text style API in examples. Scoped to `text-fonts` for now; the wider catalog still uses the legacy `fill` alias in ~40 unrelated examples (tracked as a follow-up).
