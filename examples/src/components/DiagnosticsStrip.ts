...
export class DiagnosticsStrip extends LitElement {
    ...
    public render(): ReturnType<LitElement['render']> {
        ...
        return html`
            <div class="strip" data-severity=${top.severity} role="status" aria-live="polite" aria-atomic="true">
                <span class="severity-dot" data-severity=${top.severity} aria-hidden="true"></span>
                ${this._renderCounts(counts)}
                <span class="separator" aria-hidden="true">·</span>
                <span class="message">
                    ${top.code ? html`<code class="code">ts(${top.code})</code>` : nothing}
    ...
    private _renderCounts(counts: Record<EditorDiagnosticSeverity, number>): ReturnType<LitElement['render']> {
    ...
    private _onJump(diagnostic: EditorDiagnostic): void {
        this.dispatchEvent(
            new CustomEvent<DiagnosticJumpEvent>('diagnostic-jump', {
                detail: {
