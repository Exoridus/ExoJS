...
export class NavigationSection extends LitElement {
    ...
    public render(): ReturnType<LitElement['render']> {
        return html`
            <section>
                <button class="toggle" type="button" aria-expanded=${String(this.expanded)} @click=${this._onToggle}>
                    <span class="title">${this.headline}</span>
                    <span class="meta">
                        ${this.unavailableCount > 0
                            ? html`<span class="count" title="${this.unavailableCount} unavailable example${this.unavailableCount === 1 ? '' : 's'}"
