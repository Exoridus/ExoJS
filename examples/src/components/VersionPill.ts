...
export class VersionPill extends LitElement {
    ...
    private _renderMenu(selected: VersionInfo): ReturnType<LitElement['render']> {
        return html`
            <div class="menu" role="menu" aria-label="ExoJS version">
                ...
                <div class="menu-foot">Versions are URL-driven: <span class="mono">?v=&amp;ex=</span></div>
            </div>
        `;
    }
    ...
    private _renderRow(version: VersionInfo, selected: VersionInfo): ReturnType<LitElement['render']> {
        ...
        return html`
            ...
            >
                ...
                ${version.summary ? html`<span class="row-summary">${version.summary}</span>` : html`<span class="row-summary"></span>`}
                ${version.latest ? html`<span class="row-latest">Latest</span>` : nothing}
            </button>
        `;
    }
    ...
    private _onSelect(id: string): void {
        this.dispatchEvent(
            new CustomEvent<SelectVersionEvent>('select-version', {
                detail: { id },
                bubbles: true,
                composed: true,
            })
