...
export class Editor extends LitElement {
    ...
    protected override willUpdate(changedProperties: Map<PropertyKey, unknown>): void {
        if (changedProperties.has('activeExample') || changedProperties.has('selectedVersionId')) {
            ...
            const newKey = newPath !== null && this.selectedVersionId ? `${this.selectedVersionId}::${newPath}` : null;
    ...
    public render(): ReturnType<LitElement['render']> {
        ...
        return html`
            ...
            <exo-diagnostics-strip .diagnostics=${this._diagnostics} @diagnostic-jump=${this._onDiagnosticJump}></exo-diagnostics-strip>
                    ...
                    ${errors.map(
                        error => html`
                            <article class="error-item">
                                ...
                                ${error.details && error.details !== error.summary ? html`<pre class="error-details">${error.details}</pre>` : nothing}
                            </article>
                        `
                    )}
                </div>
    ...
    private _getCombinedErrors(): Array<PreviewErrorEntry> {
        return [
            ...(this.catalogLoadError ? [{ summary: 'Failed to load examples catalog', details: this.catalogLoadError }] : []),
