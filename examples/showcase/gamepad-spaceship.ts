...
class GamepadSpaceshipScene extends Scene {
    ...
    override init(): void {
        ...
        this.ship = new Sprite(this.loader.get(Texture, assets.demo.textures.shipA)).setAnchor(0.5).setScale(0.5).setPosition(width / 2, height / 2);
        this.engine = this.app.audio.play(new AudioGenerator({ type: 'sawtooth', frequency: 90 }), { volume: 0 });

        this.fx = new Graphics();

        this.particles = new ParticleSystem(this.loader.get(Texture, assets.demo.textures.particleSpark), { capacity: 4000 });
