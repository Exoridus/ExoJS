...
import { quadraticCurveTo } from './math';
...
const
     ...
     * @returns {Geometry}
     */
    buildLine = function (startX, startY, endX, endY, width, vertices = [], indices = []) {
        const points = [startX, startY, endX, endY],
            distance = width / 2,
            index = vertices.length / 6,
            perpA = new Vector(startX - endX, startY - endY).perp().normalize().multiply(distance),
            perpB = new Vector(endX - startX, endY - startY).perp().normalize().multiply(distance);
     ...
     * @returns {Geometry}
     */
    buildPath = function (points, width, vertices = [], indices = []) {
        if (points.length < 4) {
     ...
     * @returns {Geometry}
     */
    buildCircle = (centerX, centerY, radius, vertices = [], indices = []) => {
     ...
     * @returns {Geometry}
     */
    buildEllipse = (centerX, centerY, radiusX, radiusY, vertices = [], indices = []) => {
     ...
     * @returns {Geometry}
     */
    buildPolygon = (points, vertices = [], indices = []) => {
        if (points.length < 6) {
     ...
     * @returns {Geometry}
     */
    buildRectangle = (x, y, width, height, vertices = [], indices = []) => {
     ...
     * @returns {Geometry}
     */
    buildStar = function (centerX, centerY, points, radius, innerRadius = radius / 2, rotation = 0) {
