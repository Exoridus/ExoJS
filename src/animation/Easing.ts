...
export class Ease {
  ...
  public static readonly quadInOut: EasingFunction = (t: number): number => (t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t);
  ...
  public static readonly cubicInOut: EasingFunction = (t: number): number => (t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2);
  ...
  public static readonly quartInOut: EasingFunction = (t: number): number => (t < 0.5 ? 8 * t * t * t * t : 1 - Math.pow(-2 * t + 2, 4) / 2);
  ...
  public static readonly quintInOut: EasingFunction = (t: number): number => (t < 0.5 ? 16 * t * t * t * t * t : 1 - Math.pow(-2 * t + 2, 5) / 2);

  public static readonly sineIn: EasingFunction = (t: number): number => 1 - Math.cos((t * Math.PI) / 2);
  public static readonly sineOut: EasingFunction = (t: number): number => Math.sin((t * Math.PI) / 2);
  public static readonly sineInOut: EasingFunction = (t: number): number => -(Math.cos(Math.PI * t) - 1) / 2;

  public static readonly expoIn: EasingFunction = (t: number): number => (t === 0 ? 0 : Math.pow(2, 10 * t - 10));
  public static readonly expoOut: EasingFunction = (t: number): number => (t === 1 ? 1 : 1 - Math.pow(2, -10 * t));
  ...
  public static readonly circIn: EasingFunction = (t: number): number => 1 - Math.sqrt(1 - Math.pow(t, 2));
  public static readonly circOut: EasingFunction = (t: number): number => Math.sqrt(1 - Math.pow(t - 1, 2));
  ...
  public static readonly bounceIn: EasingFunction = (t: number): number => 1 - bounceOutFn(1 - t);
  public static readonly bounceInOut: EasingFunction = (t: number): number => (t < 0.5 ? (1 - bounceOutFn(1 - 2 * t)) / 2 : (1 + bounceOutFn(2 * t - 1)) / 2);
