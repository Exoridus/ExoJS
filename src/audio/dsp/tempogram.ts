...
export function computeAcf(
    ...
    const acf = new Float32Array(numLags);
...
export function findTempoPeaks(
    ...
    sampleRate: number,
    topK = 3,
): Array<TempoCandidateResult> {
    const peaks: Array<TempoCandidateResult> = [];
...
export function applyTempoHysteresis(
    candidates: Array<TempoCandidateResult>,
    currentBpm: number,
    currentScore: number,
): number {
    ...
    const isOctaveRelated =
        Math.abs(top.bpm / currentBpm - 2) < 0.1 ||
        Math.abs(top.bpm / currentBpm - 0.5) < 0.05;
