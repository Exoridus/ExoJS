...
export abstract class WorkletFilter extends AudioFilter {
    ...
    protected _setAudioParam(name: string, value: number): void {
        ...
        if (param) {
            param.setTargetAtTime(value, this._workletNode.context.currentTime, 0.01);
        }
    }
    ...
    private _setup(audioContext: AudioContext): void {
        ...
        this._ready = registerWorkletProcessor(audioContext, this._workletName, this._workletSource)
            .then(() => {
