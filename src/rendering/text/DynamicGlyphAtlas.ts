...
export class DynamicGlyphAtlas {
  ...
  public constructor(width = 1024, height = 1024) {
    ...
    const canvas =
      typeof OffscreenCanvas !== 'undefined' ? (new OffscreenCanvas(width, height) as unknown as HTMLCanvasElement) : document.createElement('canvas');
   ...
   */
  public getGlyph(char: string, family: string, size: number, weight: string | number, style: 'normal' | 'italic'): GlyphInfo {
  ...
  private _rasterize(char: string, family: string, size: number, weight: string | number, fontStyle: 'normal' | 'italic', _key: GlyphKey): GlyphInfo {
    const ctx = this._ctx;
